/* KokMatch v6.43 early resume guard: must run before legacy listeners. */
(()=>{
'use strict';
...
window.__kokmatchStandalone='6.43';
window.__kokmatchVersionLock='6.43';
window.__kokmatchLegacyAutoUpdateDisabled=true;
try{sessionStorage.setItem('kokmatch_runtime_version','6.43')}catch{}
...
function shuttleSvg51(kind){
 ...
 return `<svg viewBox="0 0 24 24" role="img" aria-hidden="true">${defs}
  ...
  <path d="M8.95 16.43 10.25 17.08 22.15 9.2 21.15 6.35Z" fill="${secondary}" stroke="${dark}" stroke-width=".38"/>
...
const BUILD634=Object.freeze({version:'6.43',label:'콕매치 v6.43 · 최신 운영본'});
...
/* KokMatch v6.43: role-safe canonical member paging */
(()=>{
'use strict';
...
try{memberControls=controlsHtml637;window.memberControls=memberControls}catch(e){console.warn('KokMatch v6.43 memberControls source',e)}
...
try{
 ...
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.43 renderMembers wrapper',e)}
try{
...
}catch(e){console.warn('KokMatch v6.43 member pager wrapper',e)}
function arm637(){
...
/* KokMatch v6.43: no-flash member roster resume */
(()=>{
'use strict';
...
/* KokMatch v6.43: preflight + atomic conflict recovery for multi-organizer game creation */
(()=>{
'use strict';
...
/* KokMatch v6.43: unified white cards + grade stripe + faint grade watermark. */
(()=>{
'use strict';
