...
window.__kokmatchIOSMemberFormFix='30.0';
document.documentElement.dataset.kokmatchIOSMemberFormFix='30.0';
let boundSheet=null,lastSave=0;
function trace(s){try{window.__kokmatchV30Last=String(s||'');if(!location.pathname.includes('ios-diagnostic'))return;let el=document.getElementById('kokmatchV30Status')||document.getElementById('kokmatchV29Status');if(!el){el=document.createElement('div');el.id='kokmatchV30Status';el.style.cssText='position:fixed;left:8px;right:8px;bottom:calc(74px + env(safe-area-inset-bottom));z-index:100003;background:#172033ee;color:#fff;border-radius:10px;padding:8px 10px;font:700 11px/1.35 -apple-system,BlinkMacSystemFont,sans-serif;pointer-events:none';document.body.appendChild(el)}el.textContent='v30.0 · '+String(s||'')}catch{}}
...
function fireSelect(sel){
 if(!sel)return;const cur=String(sel.value||'');const options=[...sel.options].map(o=>({value:o.value,text:o.textContent||o.value}));
 let box=sel.nextElementSibling;if(!box||!box.classList?.contains('iosChoice30')){box=document.createElement('div');box.className='iosChoice30';box.style.cssText='display:flex;flex-wrap:wrap;gap:7px;margin-top:7px';sel.insertAdjacentElement('afterend',box)}
 ...
 box.querySelectorAll('button').forEach(b=>{b.addEventListener('touchstart',ev=>{try{ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation()}catch{};sel.value=String(b.dataset.v30||'');try{sel.dispatchEvent(new Event('change',{bubbles:true}))}catch{};fireSelect(sel);trace((sel.previousElementSibling?.textContent||sel.id)+' 변경 · '+sel.value)},{passive:false});b.addEventListener('click',ev=>{ev.preventDefault();sel.value=String(b.dataset.v30||'');try{sel.dispatchEvent(new Event('change',{bubbles:true}))}catch{};fireSelect(sel)})});
}
...
function bind(){
 ...
 if(boundSheet!==sheet){boundSheet=sheet;trace('수정폼 보정 준비')}
 bindInput(document.getElementById('fmName'));bindInput(document.getElementById('fmYear'));
 ['fmGender','fmAge','fmCls','fmType','fmRole'].forEach(id=>fireSelect(document.getElementById(id)));
 const save=[...sheet.querySelectorAll('button')].find(b=>String(b.textContent||'').trim()==='저장'||String(b.textContent||'').trim()==='등록');
