...
window.__kokmatchIOSImmediateActions='29.0';
document.documentElement.dataset.kokmatchIOSImmediateActions='29.0';
const MULTI='https://wjelumpbjklfrdjxbesj.supabase.co/functions/v1/kokmatch-multi-api';
let lastKey='',lastAt=0;
function trace(msg){try{window.__kokmatchV29Last=String(msg||'');if(!location.pathname.includes('ios-diagnostic'))return;let el=document.getElementById('kokmatchV29Status')||document.getElementById('kokmatchV28Status')||document.getElementById('kokmatchV27Status')||document.getElementById('kokmatchV26Status');if(!el){el=document.createElement('div');el.id='kokmatchV29Status';el.style.cssText='position:fixed;left:8px;right:8px;bottom:calc(74px + env(safe-area-inset-bottom));z-index:100002;background:#172033ee;color:#fff;border-radius:10px;padding:8px 10px;font:700 11px/1.35 -apple-system,BlinkMacSystemFont,sans-serif;pointer-events:none';document.body.appendChild(el)}el.textContent='v29.0 · '+String(msg||'')}catch{}}
...
function pager(btn){const raw=String(btn?.getAttribute('onclick')||''),m=raw.match(/memberPageGo46\((\d+)\)/);if(!m)return;const p=Number(m[1]);if(dedupe('page:'+p))return;trace('회원명부 '+p+'페이지');try{if(typeof window.memberPageGo46==='function')window.memberPageGo46(p)}catch(e){trace('페이지 오류 · '+(e?.message||String(e)))}}
