...
let tries=0;const timer=setInterval(()=>{if(window.__kokmatchV54Fix19){clearInterval(timer);return}if(window.__kokmatchV54Fix6){clearInterval(timer);const add=(src,onload)=>{const s=document.createElement('script');s.src=src;s.async=false;s.onload=onload||null;s.onerror=()=>console.error('콕매치 회원명부/프로필 보정 로드 실패',src);document.body.appendChild(s)};const load19=()=>window.__kokmatchV54Fix19?null:add('/app-v5.4-fix19.js?v=19.0&t='+Date.now());const load18=()=>window.__kokmatchV54Fix18?load19():add('/app-v5.4-fix18.js?v=18.2&t='+Date.now(),load19);const load9=()=>window.__kokmatchV54Fix9?load18():add('/app-v5.4-fix9.js?v=9.0&t='+Date.now(),load18);if(window.__kokmatchV54Fix8)load9();else add('/app-v5.4-fix8.js?v=8.0&t='+Date.now(),load9);return}if(++tries>=160)clearInterval(timer)},25);
})();