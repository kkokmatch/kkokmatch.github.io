...
function mark54(){document.title='콕매치 v5.4';document.documentElement.dataset.kokmatchVersion='5.4';const old=document.getElementById('currentVersion52');if(old&&old.textContent!=='v5.4'){const v=old.cloneNode(true);v.textContent='v5.4';old.replaceWith(v)}syncLatest54()}
const rq54=renderQueue;renderQueue=function(){restoreDraft54();const r=rq54();style54();decorateComposer54();decoratePhotoGender54();mark54();return r};
const ra54=renderAll;renderAll=function(){restoreDraft54();const r=ra54();style54();try{decorateComposer54();decoratePhotoGender54()}catch{}mark54();return r};

if(typeof window.changeProfile53==='function'){
 const oldChange=window.changeProfile53;
 window.changeProfile53=async function(input){if(!input?.files?.[0])return;const r=await oldChange(input);await new Promise(res=>requestAnimationFrame(()=>requestAnimationFrame(res)));try{renderMembers();renderQueue();if(currentView==='settings')renderSettings()}catch{}mark54();return r};
}

style54();restoreDraft54();setTimeout(()=>{mark54();try{renderQueue()}catch{}},0);setInterval(()=>{mark54();syncLatest54()},30000);
})();