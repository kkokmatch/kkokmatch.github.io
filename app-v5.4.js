...
function decorateComposer54(){const box=typeof $==='function'?$('queue'):document.getElementById('queue');if(!box)return;const slots=[...box.querySelectorAll('.composer54 .slot54')];slots.forEach((slot,i)=>{slot.querySelectorAll('.genderPerson54,.genderAvatar39,.profileAvatar53,.queueProfile53,.v54genderText').forEach(x=>x.remove());const id=Array.isArray(draft)?draft[i]:null,m=id&&typeof M==='function'?M(id):null;if(!m)return;const name=slot.querySelector('.slotName53');if(name)name.insertAdjacentHTML('afterbegin',genderText54(m));const grade=name?.querySelector('.gradeBadge50,.tag');if(grade){grade.style.marginLeft='0';grade.style.marginRight='0'}})}
...
if(typeof window.changeProfile53==='function'){
 ...
 window.changeProfile53=async function(input){
  ...
  const r=await oldChange(input);
  await new Promise(res=>requestAnimationFrame(()=>requestAnimationFrame(res)));
  try{renderMembers();renderQueue();if(currentView==='settings')renderSettings()}catch{}
  return r;
 };
}
