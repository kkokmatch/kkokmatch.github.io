...
window.deleteProfile53=async function(){if(!me?.memberId)return;try{await profileJson53(PROFILE53,{method:'POST',headers:{'content-type':'application/json',authorization:'Bearer '+T},body:JSON.stringify({action:'delete_profile',groupId:currentGroupId}),cache:'no-store'});delete profiles53[String(me.memberId)];profileGroup53=String(currentGroupId);renderAll();if(currentView==='settings')renderSettings()}catch(e){if(typeof showError==='function')showError(e)}};
