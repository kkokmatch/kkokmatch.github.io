...
const viewports=[{name:'fold-cover',width:344,height:882,isMobile:true},{name:'iphone',width:390,height:844,isMobile:true},{name:'galaxy',width:412,height:915,isMobile:true},{name:'tablet',width:768,height:1024,isMobile:false}];
...
async function assertRoster(page,label,width,roleKey,identity){
 const result=await page.evaluate(({width,roleKey,selfId})=>{
  ...
  for(const card of cards){
   ...
   if(ir&&ir.right>ar.left+2){const cs=getComputedStyle(info),as=getComputedStyle(actions);failures.push(`${width<600?'phone':'tablet'} info/actions overlap id=${cardId(card)} info=${Math.round(ir.left)}-${Math.round(ir.right)} action=${Math.round(ar.left)}-${Math.round(ar.right)} card=${Math.round(cr.left)}-${Math.round(cr.right)} infoWidth=${cs.width} infoGrid=${cs.gridColumnStart}/${cs.gridColumnEnd} actionWidth=${as.width} actionGrid=${as.gridColumnStart}/${as.gridColumnEnd}`)}const as=getComputedStyle(actions);if(as.gridColumnStart&&as.gridColumnStart!=='3')failures.push(`action rail not in column 3: ${as.gridColumnStart}`)
   ...
   for(const b of buttons){
    ...
    if(r.width<(width<360?33:width<600?37:44))failures.push('button too narrow');
    prev=r;
   }
...
console.log('PASS Fold cover 344px + iPhone 390px + Galaxy 412px + tablet 768px right-side roster rail geometry');
