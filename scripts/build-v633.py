...
function cleanLegacyVersionUi633(){
 try{
  ...
  const nodes=[];let n=null;while((n=walker.nextNode()))nodes.push(n);
  for(const node of nodes){
   const raw=String(node.nodeValue||''),next=raw.replace(/-?v(?:3\.7|4\.3|4\.4|6\.20)(?=\b)/g,'');
...
function rosterNeedsCanonical633(){
 ...
 for(const card of box.querySelectorAll('.memberCard')){
  const any=[...card.querySelectorAll('.memberBtns button')];
  if(any.some(b=>String(b.textContent||'').trim()==='운동'))return true;
  if(any.length&&any.some(b=>!b.classList.contains('kmRosterAction621')))return true;
 }
 return false;
}
function paintRosterCanonical633(force=false){
 try{
  if(currentView!=='members')return;
  if(!force&&!rosterNeedsCanonical633())return;
  if(typeof window.__kokmatchPaintRosterControls632==='function')window.__kokmatchPaintRosterControls632();
  else if(typeof window.__kokmatchRepairRosterV6==='function')window.__kokmatchRepairRosterV6();
 }catch(e){console.warn('KokMatch v6.33 roster canonical paint',e)}
}
function scheduleRosterCanonical633(force=false){
 clearTimeout(rosterTimer633);
 rosterTimer633=setTimeout(()=>{paintRosterCanonical633(force);setTimeout(()=>paintRosterCanonical633(false),90);setTimeout(()=>paintRosterCanonical633(false),260)},0);
}
try{
...
try{
 const baseRenderMembers633=renderMembers;
 renderMembers=function(...args){const r=baseRenderMembers633.apply(this,args);scheduleRosterCanonical633(true);return r};
 window.renderMembers=renderMembers;
}catch{}
try{
 const baseSubmitLogin633=submitLogin;
 submitLogin=async function(...args){const r=await baseSubmitLogin633.apply(this,args);scheduleRosterCanonical633(true);setTimeout(()=>scheduleRosterCanonical633(true),450);return r};
...
async function hardReload633(target=CURRENT633,reason='update'){
 ...
 const u=new URL(location.origin+'/');let standalone=false;try{standalone=window.matchMedia?.('(display-mode: standalone)')?.matches===true}catch{}u.searchParams.set('source',standalone?'pwa':'web');u.searchParams.set('kmv',String(target||CURRENT633));u.searchParams.set('r',Date.now().toString(36));
...
window.__kokmatchPaintRosterCanonical633=paintRosterCanonical633;
...
setTimeout(()=>{patchVersion633();scheduleRosterCanonical633(false)},600);
setTimeout(()=>{patchVersion633();scheduleRosterCanonical633(false)},1400);
})();
'''
