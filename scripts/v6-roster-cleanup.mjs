...
// The oldest standalone renderer did not put a member id on each card. The canonical
// roster finalizer needs a stable id to replace the old game-count/pair UI, so stamp it
// at render time and also keep an index fallback inside the finalizer for resilience.
src=src.replaceAll('<div class="card memberCard">${avatar(m)}','<div class="card memberCard" data-member-id22="${esc(m.id)}">${avatar(m)}');
...
section=section.replace(/\nwindow\.__kokmatchRosterCanonicalV6='[^']+';\ndocument\.documentElement\.dataset\.kokmatchRoster='[^']+';/g,'');
...
section=section.replace('>총관리자</span>','>개발자</span>').replace('>모임관리자</span>','>모임장</span>').replace('>게임편성자</span>','>운영진</span>').replace('>임시편성자</span>','>편성자</span>').replace('>일반회원</span>','>일반</span>');
...
section=section.slice(0,pv)+infoFn+section.slice(fn);
const oldMap=` const cards=[...box.querySelectorAll('.memberCard')],seen=[];\n cards.forEach(card=>{\n  const id=String(card.dataset.memberId46||card.dataset.memberId22||'');const m=getMember22(id);if(!m)return;`;
const newMap=` const cards=[...box.querySelectorAll('.memberCard')],seen=[];\n cards.forEach((card,index)=>{\n  let id=String(card.dataset.memberId46||card.dataset.memberId22||card.dataset.memberId||'');let m=id?getMember22(id):null;\n  if(!m&&Array.isArray(S?.members)){m=S.members[index]||null;id=String(m?.id||'')}\n  if(!m||!id)return;`;
if(!section.includes(oldMap)&&!section.includes('cards.forEach((card,index)=>'))throw new Error('Roster card mapping block not found');
section=section.replace(oldMap,newMap);
if(!section.includes('cards.forEach((card,index)=>')||!section.includes("m=S.members[index]||null"))throw new Error('Roster fallback mapping not installed');
src=src.slice(0,start)+section+src.slice(end);
...
const block=[cb,
'#members .memberCard{grid-template-columns:46px minmax(0,1fr) 90px!important;column-gap:5px!important;align-items:center!important;padding:10px 4px 10px 8px!important;overflow:visible!important}',
...
'#members .memberName45{display:inline-block!important;flex:0 0 auto!important;min-width:4.2em!important;max-width:none!important;width:auto!important;overflow:visible!important;text-overflow:clip!important;white-space:nowrap!important;font-size:14px!important;line-height:1.25!important}',
...
'#members .gamecnt,#members .recordBtn73,#members .pairBtn:not(.partnerSetBtn66),#members .memberRecordActions73{display:none!important}',
...
'#members .memberActions48,#members .v6MemberActions,#members .memberActions60,#members .memberActions65,#members .memberActions48 .status,#members .v6MemberActions .status,#members .memberActions60 .status,#members .memberActions65 .status,#members .memberBtns,#members .memberBtns65{width:90px!important;min-width:90px!important;max-width:90px!important;justify-self:end!important;margin-left:auto!important}',
'#members .memberBtns,#members .memberBtns65{justify-content:flex-end!important;gap:3px!important}#members .memberBtns .btn,#members .memberBtns65 .btn{padding:6px 4px!important;font-size:9.8px!important;white-space:nowrap!important}',
...
'@media(max-width:430px){#members .memberCard{grid-template-columns:42px minmax(0,1fr) 84px!important;column-gap:4px!important;padding:9px 3px 9px 6px!important}#members .memberActions48,#members .v6MemberActions,#members .memberActions60,#members .memberActions65,#members .memberActions48 .status,#members .v6MemberActions .status,#members .memberActions60 .status,#members .memberActions65 .status,#members .memberBtns,#members .memberBtns65{width:84px!important;min-width:84px!important;max-width:84px!important}#members .memberName45{font-size:13.5px!important;min-width:4.2em!important}#members .memberMainLine45{gap:3px!important}#members .memberMainLine45 .tag,#members .memberMainLine45 .roleBadge{font-size:8.8px!important;padding:2px 3px!important}#members .memberMetaV6,#members .memberAttendanceV6,#members .rosterPartnerBtnV6{font-size:9.8px!important}#members .memberRosterFooterV6{gap:6px!important}#members .memberBtns .btn,#members .memberBtns65 .btn{padding:6px 3px!important;font-size:9.2px!important}}',
'@media(max-width:359px){#members .memberCard{grid-template-columns:39px minmax(0,1fr) 80px!important;column-gap:3px!important;padding-left:5px!important;padding-right:2px!important}#members .memberActions48,#members .v6MemberActions,#members .memberActions60,#members .memberActions65,#members .memberActions48 .status,#members .v6MemberActions .status,#members .memberActions60 .status,#members .memberActions65 .status,#members .memberBtns,#members .memberBtns65{width:80px!important;min-width:80px!important;max-width:80px!important}#members .memberName45{font-size:12.8px!important;min-width:4.2em!important}#members .memberMainLine45 .tag,#members .memberMainLine45 .roleBadge{font-size:8.2px!important;padding:1px 3px!important}#members .memberMetaV6,#members .memberAttendanceV6,#members .rosterPartnerBtnV6{font-size:9.2px!important}}',ce].join('\n');
css=css.trimEnd()+'\n\n'+block+'\n';fs.writeFileSync(JS,src);fs.writeFileSync(CSS,css);console.log('Canonical v6 roster 6.0.4 installed.');
