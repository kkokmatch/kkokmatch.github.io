...
const canonical='/* v6.0 canonical member roster */';
...
start=src.indexOf(canonical);
let end=src.indexOf('/* v6.0 canonical interaction core',start);
if(end<0)end=src.lastIndexOf("\nwindow.__kokmatchStandalone='6.0';");
if(end<=start)throw new Error('Roster section end not found');
...
section=section.replace("if(r==='manager')return '<span class=\"roleBadge role-manager\">모임관리자</span>';","if(r==='manager')return '<span class=\"roleBadge role-manager\">모임장</span>';");
section=section.replace("if(r==='organizer')return '<span class=\"roleBadge role-organizer\">게임편성자</span>';","if(r==='organizer')return '<span class=\"roleBadge role-organizer\">운영진</span>';");
section=section.replace("try{if(typeof isTemp==='function'&&isTemp(m))return '<span class=\"roleBadge role-temp\">임시편성자</span>'}catch{}","try{if(typeof isTemp==='function'&&isTemp(m))return '<span class=\"roleBadge role-temp\">편성자</span>'}catch{}");
section=section.replace("return '<span class=\"roleBadge role-member44\">일반회원</span>';","return '<span class=\"roleBadge role-member44\">일반</span>';");

const pv=section.indexOf('function patchVisibleInfo22(card,m){');
const fn=section.indexOf('\nfunction finalizeRoster22()',pv);
if(pv<0||fn<0)throw new Error('Roster visible-info function not found');
const replacement=[
...
" if(line){line.classList.add('memberMainLine45');line.innerHTML=\"<span class='memberName45'>\"+e22(m.name)+\"</span>\"+grade22(m)+roleBadge22(m)}",
...
" const footer=document.createElement('div');footer.className='memberRosterFooterV6';footer.innerHTML=\"<span class='memberAttendanceV6'>\"+monthLabel22()+' 출석 '+attendanceCount22(m)+\"회</span>\"+(canPartner22(m)?\"<button type='button' class='pairBtn partnerSetBtn66 rosterPartnerBtnV6' onclick=\\\"openPartner66('"+jsId22(m.id)+"')\\\">파트너 설정</button>\":'');info.appendChild(footer);",
" card.dataset.gradeV6=String(m?.cls||'C').trim().toUpperCase();card.dataset.memberId22=String(m?.id||'');card.dataset.memberId=String(m?.id||'');",
" card.querySelectorAll('.v54genderText,.genderMark53').forEach(x=>x.remove());",
"}"
].join('\n');
section=section.slice(0,pv)+replacement+section.slice(fn);

src=src.slice(0,start)+section+src.slice(end);

const reBegin='/* V6_ROSTER_REENTRY_BEGIN */',reEnd='/* V6_ROSTER_REENTRY_END */';
let ra=src.indexOf(reBegin),rb=src.indexOf(reEnd);if(ra>=0&&rb>ra)src=src.slice(0,ra)+src.slice(rb+reEnd.length);
const reentry=[reBegin,
"(()=>{'use strict';let busy=false;function repair(){if(busy||currentView!=='members')return;busy=true;try{const q=String(document.getElementById('memberSearchInput46')?.value||'').trim();if(!q&&typeof window.resetMemberList46==='function')window.resetMemberList46();else if(typeof renderMembers==='function')renderMembers();try{window.__kokmatchFinalizeRoster22?.()}catch{}}catch(e){console.warn('v6 roster reentry',e)}finally{setTimeout(()=>{busy=false},100)}}const before=goView;goView=function(id,...args){const was=currentView,r=before(id,...args);if(id==='members'&&was!=='members'){queueMicrotask(repair);requestAnimationFrame(repair);setTimeout(repair,80)}return r};window.goView=goView;})();",
reEnd].join('\n');
const corePos=src.indexOf('/* v6.0 canonical interaction core');if(corePos<0)throw new Error('Interaction core missing');src=src.slice(0,corePos)+reentry+'\n\n'+src.slice(corePos);

const cb='/* V6_CANONICAL_ROSTER_CSS_BEGIN */',ce='/* V6_CANONICAL_ROSTER_CSS_END */';
let ca=css.indexOf(cb),cz=css.indexOf(ce);if(ca>=0&&cz>ca)css=css.slice(0,ca)+css.slice(cz+ce.length);
const block=[cb,
...
'#members .memberActions60 .status,#members .memberActions65 .status,#members .memberBtns,#members .memberBtns65{width:104px!important;min-width:104px!important;max-width:104px!important}',
'#members .memberBtns,#members .memberBtns65{justify-content:flex-end!important;gap:3px!important}',
...
css=css.trimEnd()+'\n\n'+block+'\n';
fs.writeFileSync(JS,src);fs.writeFileSync(CSS,css);
console.log('Canonical v6 roster installed.');
