...
async function run(engine,label){
 ...
 await page.evaluate(({s,user,group})=>{
   T='qa-token';currentGroupId='qa';S=JSON.parse(JSON.stringify(s));window.S=S;me=user;window.me=me;window.group=group;groups=[];normalizeClient();document.getElementById('login')?.classList.add('hide');renderAll();goView('members');window.__kokmatchSyncDevFrame661?.();
 },{s:base,user,group});
 await page.waitForFunction(()=>{const f=document.querySelector('#members .devChallenger659 > img.devFrame661');return !!f&&f.complete&&f.naturalWidth>0},{timeout:8000});
 ...
 const settings=await page.evaluate(()=>{
   ...
   if(!frame||!host)return null;const fr=frame.getBoundingClientRect(),hr=host.getBoundingClientRect();return {naturalWidth:frame.naturalWidth,fw:fr.width,fh:fr.height,hw:hr.width,hh:hr.height,overflow:getComputedStyle(host).overflow};
 });
 if(!settings||settings.naturalWidth<100||settings.fw<settings.hw*1.5||settings.overflow!=='visible')throw new Error(label+' settings profile frame not visible '+JSON.stringify(settings));
 ...
 console.log(`PASS ${label} v6.61 real DOM developer frame decoded and visible`);
 await browser.close();
}
