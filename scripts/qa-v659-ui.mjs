...
const managerUser={memberId:'mgr',displayName:'모임장',role:'manager',globalAdmin:false,tempOrganizer:false,groupId:'qa'};
...
async function run(engine,label){
 ...
 const page=await context.newPage();let server=state(false),actor=managerUser;
 ...
 await context.route('https://wjelumpbjklfrdjxbesj.supabase.co/functions/v1/**',async route=>{
   ...
   if(path.endsWith('/kokmatch-auto-v656')){
     if(body.action==='set')server.autoGame={enabled:body.enabled===true,mode:'ai_optimal',updatedAt:Date.now(),updatedBy:{memberId:actor.memberId,name:actor.displayName,role:actor.role,roleLabel:actor.role==='manager'?'모임장':'개발자'}};
   ...
   if(path.endsWith('/kokmatch-state-v46'))return route.fulfill({status:200,contentType:'application/json',headers,body:JSON.stringify({data:clone(server),memberCount:server.members.length,activeMemberCount:server.members.length,user:actor,group,groups:[],compact:true})});
   return route.fulfill({status:200,contentType:'application/json',headers,body:JSON.stringify({success:true,data:clone(server),user:actor,group,groups:[],members:clone(server.members),memberCount:server.members.length})});
 });
