...
window.__kokmatchModalCancelFix='28.2';
document.documentElement.dataset.kokmatchModalCancelFix='28.2';
let last28=0;
function trace28(msg){try{window.__kokmatchV28Last=String(msg||'');if(!location.pathname.includes('ios-diagnostic'))return;let el=document.getElementById('kokmatchV28Status')||document.getElementById('kokmatchV27Status')||document.getElementById('kokmatchV26Status');if(!el){el=document.createElement('div');el.id='kokmatchV28Status';el.style.cssText='position:fixed;left:8px;right:8px;bottom:calc(74px + env(safe-area-inset-bottom));z-index:100001;background:#172033ee;color:#fff;border-radius:10px;padding:8px 10px;font:700 11px/1.35 -apple-system,BlinkMacSystemFont,sans-serif;pointer-events:none';document.body.appendChild(el)}el.textContent='v28.2 · '+String(msg||'')}catch{}}
...
document.addEventListener('click',ev=>{const btn=cancelButton28(ev.target);if(!btn)return;if(Date.now()-last28<800){try{ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation()}catch{};return}fire28(ev)},{capture:true});
trace28('모달 취소 보정 준비');
})();