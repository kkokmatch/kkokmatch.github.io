...
window.__kokmatchIOSMemberSlotFix='27.0';
document.documentElement.dataset.kokmatchIOSMemberSlotFix='27.0';
...
function slotAt27(ev,x,y){
 const all=candidates27(ev,x,y);
 for(const el of all){const slot=el.closest?.('.memberBtnSlot65');if(slot&&slot.closest?.('#members .memberCard'))return slot}
 for(const el of all){const box=el.closest?.('#members .memberBtns');if(!box)continue;for(const slot of box.querySelectorAll('.memberBtnSlot65')){const r=slot.getBoundingClientRect();if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom)return slot}}
 return null;
}
function actionFromSlot27(slot){
 if(!slot)return null;const card=slot.closest?.('#members .memberCard');if(!card)return null;const id=cardId27(card);if(!id)return null;
 const btn=slot.querySelector('button');if(!btn)return null;
 const text=String(btn.textContent||'').trim();let kind='',mode='';
 if(text==='수정'||slot.classList.contains('memberBtn-edit65'))kind='edit';
 ...
 if(!kind)return null;return{id,kind,mode,text,card,slot,btn};
}
function actionAt27(ev,x,y){return actionFromSlot27(slotAt27(ev,x,y))}
...
window.addEventListener('touchstart',ev=>{
 ...
 const a=actionAt27(ev,sx27,sy27);if(a)trace27('회원버튼 터치 감지 · '+a.text+' · '+a.id);
