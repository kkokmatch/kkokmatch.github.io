...
const renderSettingsPrev51=renderSettings;renderSettings=function(){const r=renderSettingsPrev51();const box=typeof $==='function'?$('settings'):null;if(box&&me?.globalAdmin===true&&String(me?.displayName||'').trim()==='박태영'){const card=[...box.querySelectorAll('.card')].find(c=>String(c.textContent||'').includes('프로그램 버전'));if(card){const m=card.querySelector('.meta');if(m)m.textContent='콕매치 v5.4 · 실사용 QA 안정화 · 최신버전 고정'}}return r};
...
(()=>{if(window.__kokmatchV52Loader)return;window.__kokmatchV52Loader=true;const s=document.createElement('script');s.src='/app-v5.2.js?v=5.2&hotfix=5&t='+Date.now();s.async=false;s.onerror=()=>console.error('콕매치 v5.2 로드 실패');document.body.appendChild(s)})();