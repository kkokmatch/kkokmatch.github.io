...
await page.evaluate(()=>{window.S={members:[{id:'m1',name:'회원1',state:'out'}]};window.T='qa';window.currentGroupId='grp';window.normalizeClient=()=>{};window.renderAll=()=>{};window.__pageGo=0;window.__kokmatchMemberPage46=1;window.memberPageGo46=p=>window.__pageGo=p});
await page.addScriptTag({url:'http://127.0.0.1:4173/app-v5.4-fix29.js?v=29.1'});
await page.waitForFunction(()=>window.__kokmatchIOSImmediateActions==='29.1');
async function overlayTap(sel,scale=1){const b=page.locator(sel);await b.scrollIntoViewIfNeeded();const r=await b.boundingBox();if(!r)throw new Error('no box '+sel);const x=r.x+r.width/2,y=r.y+r.height/2;await page.evaluate(({x,y,w,h,scale})=>{document.querySelectorAll('.qa29').forEach(n=>n.remove());const o=document.createElement('span');o.className='qa29';o.style.cssText=`position:fixed;left:${x-w*scale/2}px;top:${y-h*scale/2}px;width:${w*scale}px;height:${h*scale}px;z-index:999999;background:transparent;pointer-events:auto`;document.body.appendChild(o)},{x,y,w:r.width,h:r.height,scale});await page.touchscreen.tap(x,y);await page.waitForTimeout(250);await page.evaluate(()=>document.querySelectorAll('.qa29').forEach(n=>n.remove()))}
...
await page.waitForTimeout(800);await overlayTap('.memberPager46 button',1.8);
if(await page.evaluate(()=>window.__pageGo)!==2)throw new Error('2페이지 좌표 touchstart 실패');
console.log('PASS v29.1 iPhone WebKit attendance + coordinate pagination touchstart');
await browser.close();