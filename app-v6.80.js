/* KokMatch v6.80 early resume guard: must run before legacy listeners. */
(()=>{
'use strict';
...
window.__kokmatchStandalone='6.80';
window.__kokmatchVersionLock='6.80';
window.__kokmatchLegacyAutoUpdateDisabled=true;
window.__kokmatchQueueStable672='6.80';
try{sessionStorage.setItem('kokmatch_runtime_version','6.80')}catch{}


/* v6.80: persistent group-scoped roster cache for PWA/mobile cold starts. */
(()=>{
...
async function compactState46(){
 ...
 }catch(e){console.warn('compact roster preserve v6.80',e)}
...
/* v6.80 pending ETA: read-only prediction from the current group's canonical state. */
...
/* migrated into v6.0: app-v92.js — v6.80 compacted no-op role observer */
(()=>{
roleLabel=function(r){return r==='admin'?'개발자':r==='manager'?'모임장':r==='organizer'?'운영진':'일반'};
...
async function fullStateFallback654(gid){
 ...
 try{
  ...
  return {members,memberCount:members.length,adminBadgeVisibility:String(x?.data?.adminBadgeVisibility||S?.adminBadgeVisibility||'all'),source:'state-fallback-v654'};
...
function repairMemberControlsV6(){
 // v6.80: obsolete roster rail rewriting disabled; canonical role-safe rail owns this DOM.
 return;
...
const BUILD634=Object.freeze({version:'6.80',label:'콕매치 v6.80 · 최신 운영본'});
...
/* KokMatch v6.80: role-safe canonical member paging */
(()=>{
'use strict';
...
try{memberControls=controlsHtml637;window.memberControls=memberControls}catch(e){console.warn('KokMatch v6.80 memberControls source',e)}
...
try{
 ...
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.80 renderMembers wrapper',e)}
try{
...
}catch(e){console.warn('KokMatch v6.80 member pager wrapper',e)}
function arm637(){
...
/* KokMatch v6.80: no-flash member roster resume */
(()=>{
'use strict';
...
/* KokMatch v6.80: preflight + atomic conflict recovery for multi-organizer game creation */
(()=>{
'use strict';
...
/* KokMatch v6.80: white member/queue cards + right-side queue grade stripe. */
(()=>{
'use strict';
...
/* KokMatch v6.80: personal queue grade watermark only. No card layout mutation. */
(()=>{
'use strict';
...
/* KokMatch v6.80: live operations dashboard inside 운동통계. */
(()=>{
'use strict';
...
/* v6.80: group-scoped AI auto game assignment + pending creator provenance. */
(()=>{
'use strict';
if(window.__kokmatchAutoGame656)return;
window.__kokmatchAutoGame656='6.80';
...
renderSettings=function(...args){const r=renderSettingsBefore656.apply(this,args);paintAutoSettings656();const box=document.getElementById('settings');if(box)[...box.querySelectorAll('.meta')].forEach(el=>{if(/콕매치 v6\.5[0-5]/.test(el.textContent||''))el.textContent='콕매치 v6.80 · AI 자동게임편성 · 편성자 기록'});return r};
...
async function runAuto656(force=false){
 ...
 try{
  ...
  return x;
 }catch(e){console.warn('auto game v6.80',e);return null}finally{autoBusy656=false}
}
...
/* v6.80 keep today live dashboard mounted */
setTimeout(()=>{if(me){paintAutoSettings656();decorateQueueAuto656()}armAuto656()},0);
})();


/* v6.80: waiting-screen auto controls, clean member badges, developer challenger frame, queue meta alignment. */
(()=>{
'use strict';
if(window.__kokmatchUiRefine658)return;
window.__kokmatchUiRefine658='6.80';
...
/* v6.80 final ordinary-badge, developer-frame and personal queue metadata guard. */
(()=>{
'use strict';
...
/* v6.80 compact auto control + Challenger crest profile frame. */
(()=>{
'use strict';
window.__kokmatchUiRefine659='6.80';
...
window.__kokmatchDevFrame661='6.80';
...
/* v6.80 final roster-card cleanup: no rectangular border/accent survives legacy render styles. */
(()=>{
 'use strict';
...
/* v6.80: manager gold aura + organizer silver aura */
(()=>{
'use strict';
if(window.__kokmatchRoleAura664)return;
window.__kokmatchRoleAura664='6.80';
const MANAGER_SRC='/assets/dev-prism-frame-v662.webp?v=6.80';
const ORGANIZER_SRC='/assets/organizer-silver-aura-v665.webp?v=6.80';
let queued=false;
function role664(id){
...
/* v6.80: automatic mode owns game composition until an authorized operator explicitly switches to manual. */
(()=>{
'use strict';
if(window.__kokmatchAutoManualGuard673)return;window.__kokmatchAutoManualGuard673='6.80';
...
/* v6.80: canonical always-visible developer badge + restored Challenger frame. */
(()=>{
'use strict';
if(window.__kokmatchDeveloperVisible674)return;window.__kokmatchDeveloperVisible674='6.80';
const DEV_FRAME674='/assets/dev-prism-frame-v662.webp?v=6.80';
...
/* v6.80: verified automatic-game controls + stable stats polish. */
(()=>{
'use strict';
if(window.__kokmatchFix675)return;window.__kokmatchFix675='6.80';
...
window.openAutoGameSettings656=function(...args){
 ...
 syncAuto675().catch(e=>console.warn('auto settings sync v6.80',e));
 return r;
};
window.toggleAutoGame656=async function(){
...
/* v6.80: resilient automatic-game toggle; save state and composition execution are separate. */
(()=>{
'use strict';
if(window.__kokmatchAutoToggle676)return;window.__kokmatchAutoToggle676='6.80';
...
window.toggleAutoGame656=async function(){
 ...
 try{
  try{const before=await req676('get');apply676(before);serverBefore=before?.config?.enabled===true}catch(e){console.warn('auto pre-sync v6.80',e)}
  ...
  req676('get').then(v=>{apply676(v);paint676()}).catch(e=>console.warn('auto verify v6.80',e));
  ...
  if(next){
   try{
    ...
    console.warn('auto initial tick v6.80',e);
...
window.openAutoGameSettings656=function(...args){
 ...
 sync676().catch(e=>console.warn('auto settings sync v6.80',e));
 return r;
};
...
/* v6.80: roster no-flash, restored green queue game badge, persistent auto/manual conflict modal. */
(()=>{
'use strict';
if(window.__kokmatchUiStability677)return;window.__kokmatchUiStability677='6.80';
...
/* v6.80: make legacy roster finalizer idempotent so operational polling cannot recreate avatars. */
(()=>{
'use strict';
if(window.__kokmatchRosterFinalizerStable677)return;
const original=window.__kokmatchFinalizeRoster22;
if(typeof original!=='function')return;
window.__kokmatchRosterFinalizerStable677='6.80';
let last='';
function sig677(){
 try{
...
/* v6.80: final member-view renderAll gate. Legacy v22 calls finalizeRoster22 directly, so bypass that chain for operational-only updates. */
(()=>{
'use strict';
if(window.__kokmatchMemberRenderGate677)return;
window.__kokmatchMemberRenderGate677='6.80';
...
/* v6.80: persistent live-operations shell, permanent queue game badge, inline monthly rank circles, mobile roster spacing. */
(()=>{
'use strict';
if(window.__kokmatchUiStability678)return;window.__kokmatchUiStability678='6.80';
...
/* v6.80: visible-shell resilience after PWA/service-worker updates. */
(()=>{
'use strict';
if(window.__kokmatchBootResilience679)return;window.__kokmatchBootResilience679='6.80';
...
function ensureVisibleShell679(){
 ...
 }catch(e){console.warn('KokMatch v6.80 visible shell recovery',e)}
 return false;
}
