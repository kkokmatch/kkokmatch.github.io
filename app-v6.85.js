/* KokMatch v6.85 early resume guard: must run before legacy listeners. */
(()=>{
'use strict';
...
window.__kokmatchStandalone='6.85';
window.__kokmatchVersionLock='6.85';
window.__kokmatchLegacyAutoUpdateDisabled=true;
window.__kokmatchQueueStable672='6.85';
try{sessionStorage.setItem('kokmatch_runtime_version','6.85')}catch{}


/* v6.85: persistent group-scoped roster cache for PWA/mobile cold starts. */
(()=>{
...
window.__kokmatchSaveRoster654=function(g,m,b='hidden'){
 try{if(!g||!Array.isArray(m)||!m.length)return false;const c={groupId:String(g),at:Date.now(),members:m.map(x=>({...x})),adminBadgeVisibility:String(b||'hidden')};window.__kokmatchFullRosterCache654=c;localStorage.setItem(K654(g),JSON.stringify(c));return true}catch{return false}
};
...
let dailyCountCacheRef651=null,dailyCountCacheLen651=-1,dailyCountCache651=new Map();
...
function renderShell(){document.body.innerHTML=`<div id="login" class="login"><div class="loginBox" id="loginBox"></div></div><div class="app"><header class="top"><div class="toprow"><div><div class="brand">🏸 콕매치</div><div class="groupLine"><button id="groupBtn" class="groupBtn" onclick="openGroupSwitch()">모임 선택</button></div><div id="who" class="who">-</div></div><button class="logout" onclick="logout()">로그아웃</button></div><div class="summary"><div><b id="sm">0</b><span>오늘 입장</span></div><div><b id="sw">0</b><span>게임대기</span></div><div><b id="sg">0</b><span>게임중</span></div></div></header><main><section id="members" class="view on"></section><section id="queue" class="view"></section><section id="playing" class="view"></section><section id="stats" class="view"></section><section id="settings" class="view"></section><section id="groups" class="view"></section></main></div><nav id="nav"></nav><div id="modal" class="modal"><div class="sheet" id="modalSheet"></div></div>`;if(!T)renderLoginName();renderNav()}
function renderNav(){const items=[['members','👥','회원명부'],['queue','▦','게임대기'],['playing','🏸','게임중'],['stats','▥','운동통계'],['settings','⚙️','설정']];if(canManageGroups())items.push(['groups','🏢','모임관리']);const n=$('nav');if(!n)return;n.className='n'+items.length;n.innerHTML=items.map(([id,ic,tx])=>`<button data-v="${id}" class="${currentView===id?'on':''}" onclick="goView('${id}')"><i>${ic}</i>${tx}</button>`).join('')}
...
async function reloginLatest(failedToken=''){
...
async function request(apiName,method='GET',body=null,params={}){const requestToken=String(T||'');const u=new URL(API);u.searchParams.set('api',apiName);Object.entries(params).forEach(([k,v])=>{if(v)u.searchParams.set(k,v)});const r=await fetch(u,{method,headers:{'content-type':'application/json',...(requestToken?{authorization:'Bearer '+requestToken}:{})},body:body?JSON.stringify(body):undefined,cache:'no-store'});const x=await r.json().catch(()=>({error:'통신 오류'}));if(!r.ok){if(r.status===401&&apiName!=='login'&&apiName!=='login_probe'){await reloginLatest(requestToken);throw new Error('로그인이 만료되었습니다.')}const e=new Error(x.error||'오류가 발생했습니다.');e.payload=x;throw e}return x}
...
function renderStats(){const box=$('stats');if(box)box.innerHTML='<div class="statsLegacyBase627"></div>'}
...
setInterval(backgroundStatePollV617,10000);
renderShell();
...
forceUpdateApp=async function(){
  ...
  }catch(e){if(b){b.disabled=false;b.textContent='↻ 새로고침'}showError(e)}
};
...
function parseBulk37(text){
  ...
  lines.forEach((line,idx)=>{
    ...
    if(!['S','A','B','C','D','E'].includes(cls))errors.push(`${idx+1}행: 급수는 S/A/B/C/D/E 중에서 입력해주세요.`);
...
async function cleanFix42(){
 ...
 const w=(async()=>{try{if('caches'in window){const ks=await caches.keys();await Promise.all(ks.map(k=>caches.delete(k)))}}catch(e){}try{if('serviceWorker'in navigator){const r=await navigator.serviceWorker.getRegistration('/');if(r)await r.update().catch(()=>{})}}catch(e){}})();
...
async function cleanupLocal43(){
 ...
 const work=(async()=>{
  ...
  try{if('serviceWorker'in navigator){const r=await navigator.serviceWorker.getRegistration('/');if(r)await r.update().catch(()=>{})}}catch(e){}
 })();
...
renderSettings=function(){
 ...
 const btn=$('forceUpdateBtn');if(btn)btn.textContent='↻ 새로고침';
};
...
(()=>{
const BULK_V44='https://wjelumpbjklfrdjxbesj.supabase.co/functions/v1/kokmatch-bulk-v646';
...
function parseBulk44(text){
 const lines=String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean),rows=[],errors=[];
 lines.forEach((line,idx)=>{const cells=(line.includes('\t')?line.split('\t'):line.split(',')).map(x=>x.trim());if(idx===0&&['이름','name'].includes((cells[0]||'').toLowerCase()))return;const [name,yr,gender,clsRaw,typeRaw='일반']=cells,year=Number(yr),cls=String(clsRaw||'').toUpperCase(),t=String(typeRaw||'일반').toLowerCase();const type=['게스트','guest'].includes(t)?'guest':'member';if(!name)errors.push(`${idx+1}행: 이름이 없습니다.`);if(!Number.isInteger(year)||year<1900||year>currentYearKst44())errors.push(`${idx+1}행: 출생연도를 확인해주세요.`);if(!['남','여'].includes(gender))errors.push(`${idx+1}행: 성별은 남 또는 여로 입력해주세요.`);if(!['S','A','B','C','D','E'].includes(cls))errors.push(`${idx+1}행: 급수는 S/A/B/C/D/E 중에서 입력해주세요.`);if(!['일반','회원','member','게스트','guest',''].includes(t))errors.push(`${idx+1}행: 구분은 일반 또는 게스트로 입력해주세요.`);rows.push({name,year,gender,cls,type,age:String(ageBand44(year))})});return{rows,errors}
}
window.submitBulk37=async function(){
...
async function compactState46(){
 ...
 try{
  ...
  if(curMembers.length&&prevGroup===requestedGroup&&(!prevExpected||curMembers.length>=prevExpected)){
   window.__kokmatchSaveRoster654?.(requestedGroup,curMembers,S?.adminBadgeVisibility);
  }
 }catch{}
 /* compact-preserve-full-roster-v655: operational polling must never destroy the canonical full roster. */
 ...
 }catch(e){console.warn('compact roster preserve v6.85',e)}
...
function stampMemberCards46(page){
 ...
 window.__kokmatchMemberPage46=memberPage46;window.__kokmatchMemberPageRequested46=memberPage46;
}
...
/* v6.34: visibility state refresh is handled by the final session coordinator. */
...
function canSeeGlobal50(){return true}
...
ageTag=function(m){
 ...
 const safe=['S','A','B','C','D','E'].includes(c)?c:'C';
...
function canSeeGlobal51(){return true}
...
function shuttleSvg51(kind){
 ...
 const cork=`<g>
  ...
  <path d="M6.35 16.79 9.15 18.18" stroke="#72685C" stroke-width="2.2" stroke-linecap="round"/>
  <path d="M6.79 16.2 9.28 17.9" stroke="#E8DDC9" stroke-width="1.15" stroke-linecap="round"/>
 </g>`;
 const frame=`<path d="M8.05 15.55 15.4 3.35M8.65 16.05 18.5 4.25M9.2 16.79 21.35 7.15" fill="none" stroke="#5F5B56" stroke-opacity=".55" stroke-width=".56" stroke-linecap="round"/>
  <path d="M8.05 15.55 9.25 16.79 10.1 17.05" fill="none" stroke="#6B6258" stroke-width=".65" stroke-linecap="round"/>`;
 if(kind==='global'){
  return `<svg viewBox="0 0 24 24" role="img" aria-hidden="true">${defs}
   <path d="M7.55 15.55 9.15 16.33 16.1 3.1 13.75 3.65Z" fill="#FF3D67" stroke="#7B2342" stroke-width=".35"/>
   <path d="M8.05 15.75 9.62 16.79 18.45 4.05 16.15 3.15Z" fill="#FFB000" stroke="#8A6100" stroke-width=".35"/>
 ...
 return `<svg viewBox="0 0 24 24" role="img" aria-hidden="true">${defs}
  ...
  <path d="M8.05 15.82 9.66 16.79 19.1 4.25 16.1 3.2Z" fill="${secondary}" stroke="${dark}" stroke-width=".38"/>
  <path d="M8.55 16.12 10.02 16.85 21.15 6.35 19.1 4.25Z" fill="${featherFill}" stroke="${dark}" stroke-width=".38"/>
  <path d="M8.95 16.79 10.25 17.08 22.15 9.2 21.15 6.35Z" fill="${secondary}" stroke="${dark}" stroke-width=".38"/>
...
function canSeeGlobal52(){return true}
...
function shuttleSvg52(kind){
 ...
 const ribs=`<path d="M8.6 15.75 15.3 3.55M9.2 16.15 18.3 4.35M9.72 16.79 21.05 6.3" fill="none" stroke="#5d5954" stroke-opacity=".58" stroke-width=".58" stroke-linecap="round"/>
 ...
 if(kind==='global'){
  return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
   ...
   <path d="M8.25 15.42C10.7 10.55 13.25 6.3 16 3.35L18.6 4.35C17.25 8.25 14.25 12.8 9.72 16.79Z" fill="#ffb000" stroke="#8a6100" stroke-width=".34"/>
   <path d="M8.75 15.8C12.1 11.4 15.55 7.4 18.6 4.35L20.55 6.2C18.75 9.6 15.2 13.35 10.05 16.85Z" fill="#20c85a" stroke="#126b36" stroke-width=".34"/>
 ...
 if(kind==='manager'){
  return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
   ...
   <path d="M8.25 15.45C10.7 10.55 13.3 6.25 16.05 3.35L18.7 4.35C17.25 8.25 14.3 12.85 9.72 16.79Z" fill="url(#${uid}_gold)" stroke="#835400" stroke-width=".38"/>
   <path d="M8.75 15.82C12.15 11.35 15.55 7.45 18.7 4.35L20.65 6.25C18.75 9.65 15.25 13.4 10.05 16.85Z" fill="url(#${uid}_gold)" stroke="#835400" stroke-width=".38"/>
 ...
 return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
  ...
  <path d="M8.25 15.45C10.7 10.55 13.3 6.25 16.05 3.35L18.7 4.35C17.25 8.25 14.3 12.85 9.72 16.79Z" fill="${colors.b}" stroke="${colors.edge}" stroke-width=".4"/>
  <path d="M8.75 15.82C12.15 11.35 15.55 7.45 18.7 4.35L20.65 6.25C18.75 9.65 15.25 13.4 10.05 16.85Z" fill="${colors.c}" stroke="${colors.edge}" stroke-width=".4"/>
...
(()=>{
function canSeeGlobal53(){return true}
...
(()=>{
function canSeeGlobal54(){return true}
...
/* v6.85 pending ETA: read-only prediction from the current group's canonical state. */
function etaTime680(value){
 if(value===null||value===undefined||value==='')return NaN;
 const n=Number(value);return Number.isFinite(n)?n:Date.parse(value);
}
function etaDay680(time){return new Date(time+4*3600000).toISOString().slice(0,10)} // KST 05:00 reset
function etaComposition680(players,members){
 const grades=(players||[]).map(id=>members.get(String(id))?.cls).map(g=>String(g||'').trim().toUpperCase());
 if(grades.length!==4||grades.some(g=>!Object.hasOwn(gradeV,g)))return {key:'unknown',prior:15};
 const same=grades.every(g=>g===grades[0]),high=grades.filter(g=>g==='A'||g==='B').length>=3;
 return same||high?{key:'balanced',prior:17.5}:{key:'mixed',prior:12.5};
}
function etaModel680(state,now){
 const members=new Map((state.members||[]).map(m=>[String(m.id),m])),seen=new Set(),samples=[];
 for(const g of state.history||[]){
  const end=etaTime680(g.endedAt),start=etaTime680(g.startedAt);
  const duration=Number.isFinite(start)?(end-start)/60000:Number(g.durationMin);
  // Thirty-minute automatic closures are censored observations, not measured play times.
  if(g.autoEnded===true||!Number.isFinite(end)||end>now||etaDay680(end)!==etaDay680(now)||!Number.isFinite(duration)||duration<3||duration>=30)continue;
  const id=g.id||g.gameId?String(g.id||g.gameId):JSON.stringify([end,start,g.players]);if(seen.has(id))continue;seen.add(id);
  samples.push({end,duration,key:etaComposition680(g.players,members).key});
 }
 samples.sort((a,b)=>a.end-b.end);
 function estimate(players){
  const composition=etaComposition680(players,members);
  if(!samples.length)return {minutes:composition.prior,samples:0,observed:[]};
  // Winsorize long/short outliers once there is enough evidence; retain recent trend.
  const sorted=samples.map(s=>s.duration).sort((a,b)=>a-b),median=sorted[Math.floor(sorted.length/2)];
  const deviations=sorted.map(v=>Math.abs(v-median)).sort((a,b)=>a-b);
  const spread=Math.max(3,3*deviations[Math.floor(deviations.length/2)]);
  const clipped=samples.map(s=>({...s,duration:samples.length>=5?Math.max(median-spread,Math.min(median+spread,s.duration)):s.duration}));
  const average=rows=>rows.reduce((sum,s)=>sum+s.duration,0)/rows.length;
  const recent=clipped.slice(-8),daily=average(clipped),trend=average(recent);
  const comparable=clipped.filter(s=>composition.key!=='unknown'&&s.key===composition.key).slice(-12);
  const comparableWeight=Math.min(.65,comparable.length/12*.65);
  const measured=(daily*.4+trend*.6)*(1-comparableWeight)+(comparable.length?average(comparable):0)*comparableWeight;
  const weight=Math.min(1,samples.length/8);
  return {minutes:composition.prior*(1-weight)+measured*weight,samples:samples.length,observed:comparable.length>=4?comparable:clipped};
 }
 return {estimate};
}
function pendingEtas680(state,now=Date.now()){
 const result=new Map(),model=etaModel680(state,now);
 const count=Math.max(0,Math.min(16,Math.floor(Number(state.courtCount)||0)));
 const courts=Array.from({length:count},()=>0);
 for(const game of state.games||[]){
  const court=Number(game.court)-1;if(!Number.isInteger(court)||court<0||court>=count)continue;
  const estimate=model.estimate(game.players),start=etaTime680(game.startedAt);
  const elapsed=Number.isFinite(start)&&start<=now?Math.max(0,(now-start)/60000):0;
  // A still-running game cannot be declared free solely because its mean has elapsed.
  const survivors=estimate.observed.filter(s=>s.duration>elapsed);
  const base=Math.max(1,estimate.minutes-elapsed),weight=Math.min(1,estimate.samples/8);
  const residual=survivors.length>=3?base*(1-weight)+survivors.reduce((n,s)=>n+s.duration-elapsed,0)/survivors.length*weight:base;
  courts[court]=Math.max(courts[court],1,residual);
 }
 for(const pending of state.pendingGames||[]){
  const players=pending.players||[];
  if(players.length!==4||players.some(id=>!id)||new Set(players.map(String)).size!==4)continue;
  if(!count){result.set(String(pending.id),{minutes:null,now});continue;}
  const earliest=Math.min(...courts),court=courts.indexOf(earliest),estimate=model.estimate(players);
  result.set(String(pending.id),{minutes:earliest,now,samples:estimate.samples});
  courts[court]=earliest+estimate.minutes;
 }
 return result;
}
function pendingEtaText680(eta){
 if(!eta)return {main:'',detail:''};
 if(eta.minutes===null)return {main:'사용 코트 확인 필요',detail:'코트가 설정되면 예상 시작을 표시합니다'};
 const mins=eta.minutes===0?0:Math.max(1,Math.round(eta.minutes));
 const time=new Intl.DateTimeFormat('ko-KR',{timeZone:'Asia/Seoul',hour:'2-digit',minute:'2-digit',hourCycle:'h23'}).format(new Date(eta.now+mins*60000));
 return {main:mins===0?'예상 곧 시작 가능':`예상 시작 약 ${mins}분 후`,detail:`예상 ${time} 시작 · ${eta.samples?'당일 경기 반영':'급수 기준'} · 코트 운영에 따라 변동`};
}
function pendingEtaHtml680(pg,eta){
 const text=pendingEtaText680(eta);
 return `<div class="pendingEta680" data-pending-eta680="${esc(String(pg.id))}"${eta?'':' hidden'}><strong>${esc(text.main)}</strong><small>${esc(text.detail)}</small></div>`;
}
function refreshPendingEtas680(){
 if(!me||document.hidden||currentView!=='queue')return;
 const etas=pendingEtas680(S);
 document.querySelectorAll('#queue [data-pending-eta680]').forEach(el=>{
  const eta=etas.get(el.dataset.pendingEta680),text=pendingEtaText680(eta);
  el.hidden=!eta;
  const strong=el.querySelector('strong'),small=el.querySelector('small');
  if(strong&&strong.textContent!==text.main)strong.textContent=text.main;
  if(small&&small.textContent!==text.detail)small.textContent=text.detail;
 });
}

window.__kokmatchRefreshPendingEtas680=refreshPendingEtas680;

function pendingCard54(pg,i,eta){
 const waited=Math.max(0,Math.floor((Date.now()-Number(pg.createdAt||Date.now()))/60000));
 const slots=Array.from({length:4},(_,idx)=>{
  const id=pg.players?.[idx],m=id?M(id):null;
...
renderQueue=function(){
 ...
 try{
  ...
  const etas=pendingEtas680(S);
  const pending=`<div class="subhead"><b>편성대기 현황</b><span class="tag">${S.pendingGames.length}조</span></div>${S.pendingGames.length?S.pendingGames.map((pg,i)=>pendingCard54(pg,i,etas.get(String(pg.id)))).join(''):'<div class="empty">편성대기 중인 조가 없습니다.</div>'}`;
  box.innerHTML=`<div class="title"><h2>게임대기</h2><span class="tag">${S.queue.length+S.pendingGames.reduce((n,g)=>n+(g.players?.length||0),0)}명</span></div>${composer}${personal}${pending}`;
  if(typeof decorateResponsive48==='function')decorateResponsive48();
...
let resetTimer71=null,registerBusy71=false,explicitLogin71=false,loginStartToken71='';
...
renderSettings=function(){
 ...
 [...box.querySelectorAll('.meta')].forEach(el=>{if((el.textContent||'').includes('콕매치 v71'))el.textContent='콕매치 v72 · 평균대기 · 설정정리'});
};
...
renderSettings=function(){
 ...
 [...box.querySelectorAll('.meta')].forEach(el=>{if((el.textContent||'').includes('콕매치 v72'))el.textContent='콕매치 v73 · 가입/출석 기록 · 리셋문구 개선'});
};
...
renderSettings=function(){renderSettings73();const box=$('settings');if(!box)return;[...box.querySelectorAll('.meta')].forEach(el=>{if((el.textContent||'').includes('콕매치 v73'))el.textContent='콕매치 v74 · 회원정보 좌측정렬 · 연도별출석'})};

if(me)renderAll();
})();

/* V6_DISABLE_LEGACY_AUTO_UPDATE */
/* Disabled legacy auto-update section: app-v75.js */
...
(()=>{
function canSeeGlobal84(){return true}
...
(()=>{
function canSeeGlobal86(){return true}
function mine86(m){
 if(!m)return false;
 if(me?.memberId&&String(m.id)===String(me.memberId))return true;
 return !me?.memberId&&String(m.name||'').trim()===String(me?.displayName||'').trim();
...
/* migrated into v6.0: app-v92.js — v6.85 compacted no-op role observer */
(()=>{
roleLabel=function(r){return r==='admin'?'개발자':r==='manager'?'모임장':r==='organizer'?'운영진':'일반'};
...
(()=>{const rs=renderSettings;renderSettings=function(){rs();const b=$('settings');if(b)[...b.querySelectorAll('.meta')].forEach(e=>{if((e.textContent||'').includes('콕매치 v92'))e.textContent='콕매치 v93 · 편성화면 역할배지/X 위치 보정'})};if(me)renderAll();})();
...
function tintGradeContainers97(root=document){if(root?.id==='queue')return;
...
const CONTAINERS99='.memberCard73,.memberCard71,.memberCard,.playingPlayer53,.player54,.playerCard,.courtPlayer,.choiceBtn,.pickRow,.candidateRow,.partnerSearchRow82,.partnerPickedCard82,.voteMemberRow,.attendeeRow,.pollMemberRow,.memberVoteRow';
function tintAllGrades99(root=document){if(root?.id==='queue')return;[...root.querySelectorAll('.tag')].forEach(tag=>{if(tag.closest?.('#queue'))return;const txt=(tag.textContent||'').trim();if(!/^(?:\d{2})?[A-E](?:급)?$/i.test(txt))return;const cs=getComputedStyle(tag);let base=cs.backgroundColor;if(!base||base==='transparent'||/rgba\([^)]*,\s*0\)/.test(base))base=cs.borderTopColor||cs.color;const tint=rgba99(base,alphaForGrade99(txt));if(!tint)return;let container=tag.closest(CONTAINERS99);if(!container){const p=tag.parentElement;if(p&&p!==document.body)container=p.closest('button,.card,[class*="slot"],[class*="player"],[class*="member"],[class*="queue"],[class*="row"]')||p}if(container){container.style.backgroundColor=tint;container.style.transition='background-color .15s ease';container.dataset.gradeTint='99'}})}
...
/* v6.21: obsolete v99 member attendance fast-path removed. */
...
const GRADE_BOX12='.memberCard73,.memberCard71,.memberCard,.playingPlayer53,.player54,.playerCard,.courtPlayer,.choiceBtn,.pickRow,.candidateRow,.partnerSearchRow82,.partnerPickedCard82,.voteMemberRow,.attendeeRow,.pollMemberRow,.memberVoteRow';
...
async function manualRefresh16(){
 ...
 try{
  try{if('serviceWorker'in navigator){const r=await navigator.serviceWorker.getRegistration('/');if(r)await r.update().catch(()=>{})}}catch{}
  try{if('caches'in window){const ks=await caches.keys();await Promise.all(ks.map(k=>caches.delete(k)))}}catch{}
  location.reload();
 }catch(e){if(b){b.disabled=false;b.textContent='↻ 새로고침'}}
}
...
loadState=async function(...args){
 ...
 try{return await loadStatePrev16(...args)}catch(e){
  ...
  if(/만료|로그인|401/.test(msg)){if(Date.now()<Number(window.__kokmatchFreshLoginUntil630||0))throw e;showLoginInline16('로그인이 만료되어 다시 로그인이 필요합니다.');return}
  throw e;
 }
};
...
renderSettings=function(){
 ...
 const btn=typeof $==='function'?$('forceUpdateBtn'):null;if(btn){btn.textContent='↻ 새로고침';btn.onclick=manualRefresh16}
};
...
function hiddenAdmin17(){return false}
...
const GRADE_BOX17='.memberCard73,.memberCard71,.memberCard,.playingPlayer53,.player54,.playerCard,.courtPlayer,.choiceBtn,.pickRow,.candidateRow,.partnerSearchRow82,.partnerPickedCard82,.voteMemberRow,.attendeeRow,.pollMemberRow,.memberVoteRow';
...
function playingSig17(n){
 const g=S.games.find(x=>Number(x.court)===n);return JSON.stringify([courtLabel(n),canGame(),S?.adminBadgeVisibility||'hidden',viewerIsDeveloper17(),g?g.id:'',g?g.startedAt:'',g?g.players.map(memberDisplaySig17):[]]);
}
...
function queueSig17(){
 try{
  ...
  return JSON.stringify([q,p,draft,S?.adminBadgeVisibility||'hidden',canGame(),viewerIsDeveloper17()]);
...
function syncQueue17(){
 ...
 else{refreshQueueTimes17();window.__kokmatchRefreshPendingEtas680?.();stabilize17($('queue')||document)}
}
...
function navItems25(){
 const items=[['members','👥','회원명부'],['queue','▦','게임대기'],['playing','🏸','게임중'],['stats','▥','운동통계'],['settings','⚙️','설정']];
...
let loginChoices33=[],selectedLogin33=null,loginBusy33=false,loginFinalizing33=false;
...
async function finalizeLogin33(x,selection){
 T=x.token;localStorage.setItem(TOKEN_KEY,T);localStorage.setItem(SESSION_DAY_BASE_V670,authBusinessDayV670());protectFreshLogin640(T);
...
(()=>{
const BULK34_API='https://wjelumpbjklfrdjxbesj.supabase.co/functions/v1/kokmatch-bulk-v646';
...
function parseBulk34(text){const lines=String(text||'').split(/\r?\n/).map(x=>x.trim()).filter(Boolean),rows=[],errors=[];lines.forEach((line,idx)=>{const cells=(line.includes('\t')?line.split('\t'):line.split(',')).map(x=>x.trim());if(idx===0&&['이름','name'].includes((cells[0]||'').toLowerCase()))return;const [name,yr,gender,clsRaw,kindRaw='일반']=cells,year=Number(yr),cls=String(clsRaw||'').toUpperCase(),rr=parseRole34(kindRaw);if(!name)errors.push(`${idx+1}행: 이름이 없습니다.`);if(!Number.isInteger(year)||year<1900||year>yearNow34())errors.push(`${idx+1}행: 출생연도를 확인해주세요.`);if(!['남','여'].includes(gender))errors.push(`${idx+1}행: 성별은 남 또는 여로 입력해주세요.`);if(!['S','A','B','C','D','E'].includes(cls))errors.push(`${idx+1}행: 급수는 S/A/B/C/D/E 중에서 입력해주세요.`);if(!rr)errors.push(`${idx+1}행: 구분은 모임장, 운영진, 일반 또는 게스트로 입력해주세요.`);else if(rr.type!=='guest'&&!canAssign34(rr.role))errors.push(`${idx+1}행: ${roleLabel34(rr.role)} 역할을 등록할 권한이 없습니다.`);rows.push({name,year,gender,cls,type:rr?.type||'member',role:rr?.role||'member'})});return{rows,errors}}
...
async function finalizeLogin35(x,c){T=x.token;localStorage.setItem(TOKEN_KEY,T);localStorage.setItem(SESSION_DAY_BASE_V670,authBusinessDayV670());const gid=x.groupId||c?.groupId||'';if(gid){currentGroupId=gid;localStorage.setItem(GROUP_KEY,currentGroupId)}$('login').classList.add('hide');pendingLoginPin='';await loadState(true);const mine=me?.memberId?M(me.memberId):null;if(mine?.state==='out')openEntry()}
...
saveMemberNow=async function(){
 ...
 if(!['S','A','B','C','D','E'].includes(cls))return alert('급수는 S/A/B/C/D/E 중에서 선택해주세요.');
...
async function finalizeLogin37(x,c){
 T=x.token;localStorage.setItem(TOKEN_KEY,T);localStorage.setItem(SESSION_DAY_BASE_V670,authBusinessDayV670());const gid=x.groupId||c?.groupId||'';
...
saveMemberNow=async function(){
 ...
 if(!name)return alert('이름을 입력해주세요.');if(!Number.isInteger(year)||year<1900)return alert('출생연도를 확인해주세요.');if(!['남','여'].includes(gender))return alert('성별을 확인해주세요.');if(!['S','A','B','C','D','E'].includes(cls))return alert('급수는 S/A/B/C/D/E 중에서 선택해주세요.');if(rr.type==='guest'&&!inviter)return alert('게스트의 초대인을 입력해주세요.');
...
(()=>{
const ROSTER42='https://wjelumpbjklfrdjxbesj.supabase.co/functions/v1/kokmatch-roster-v654';
...
roleBadge=function(m){if(adminMember42(m)&&false&&!developer42())return generalBadge42();return roleBadgePrev42(m)};

function invalidateMembers42(){memberReady42=false;memberLoadedAt42=0}
function hasFullRoster42(){const got=Array.isArray(S?.members)?S.members.length:0,gid=String(currentGroupId||''),eg=String(window.__kokmatchMemberCountGroup46||gid),expected=eg===gid?Number(window.__kokmatchMemberCount46||0):0;return got>0&&(!expected||got>=expected)}
function cacheRoster654(gid,members,badge='all'){try{return !!window.__kokmatchSaveRoster654?.(gid,members,badge)}catch{return false}}
function restoreRoster654(gid){
 try{
  ...
  S={...(S||{}),members,adminBadgeVisibility:String(c.adminBadgeVisibility||S?.adminBadgeVisibility||'hidden')};
...
function applyRoster42(x,gid){
 ...
 S={...(S||{}),members,adminBadgeVisibility:String(x?.adminBadgeVisibility||S?.adminBadgeVisibility||'hidden')};
 window.__kokmatchMemberCount46=Number(x?.memberCount||members.length);window.__kokmatchMemberCountGroup46=String(gid);normalizeClient();memberReady42=true;memberGroup42=gid;memberLoadedAt42=Date.now();cacheRoster654(gid,members,S?.adminBadgeVisibility);return true;
}
async function fullStateFallback654(gid){
 const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),8000);
 try{
  const u=new URL(API);u.searchParams.set('api','state');u.searchParams.set('groupId',gid);u.searchParams.set('rosterFallback','654');u.searchParams.set('t',Date.now());
  ...
  return {members,memberCount:members.length,adminBadgeVisibility:String(x?.data?.adminBadgeVisibility||S?.adminBadgeVisibility||'hidden'),source:'state-fallback-v654'};
...
async function fetchRoster42(force=false){
 ...
 const promise=(async()=>{
  ...
  try{
   const x=await fullStateFallback654(gid);if(String(currentGroupId||'')!==gid)return x;applyRoster42(x,gid);return x;
  }catch(fallbackErr){
   if(hasFullRoster42()){memberReady42=true;memberGroup42=gid;memberLoadedAt42=Date.now();cacheRoster654(gid,S.members,S?.adminBadgeVisibility);return {members:S.members,memberCount:S.members.length,stale:true}}
   const msg=fallbackErr?.message||primaryErr?.message||'회원명단 연결이 원활하지 않습니다. 네트워크 상태를 확인하고 다시 시도해주세요.';
   throw new Error(msg);
  }
 })();
...
renderMembers=function(){
 if(currentView==='members'&&(!memberReady42||memberGroup42!==String(currentGroupId||''))){
  if(hasFullRoster42()){memberReady42=true;memberGroup42=String(currentGroupId||'');memberLoadedAt42=Date.now();cacheRoster654(memberGroup42,S.members,S?.adminBadgeVisibility);return renderMembersPrev42()}
  showRosterLoading42();if(!memberReq42)enterMembers42(false);return;
 }
...
async function registerSw43(){if(!supported43())return null;if(swReg43)return swReg43;swReg43=await navigator.serviceWorker.register('/kokmatch-sw.js',{scope:'/',updateViaCache:'none'});await navigator.serviceWorker.ready;return swReg43}
...
async function activeSw44(force=false){
 ...
 const initial=await navigator.serviceWorker.register('/kokmatch-sw.js',{scope:'/',updateViaCache:'none'});
...
renderMembers=function(){if(memberRendering46)return;const box=typeof $==='function'?$('members'):null,sig=memberSig46(),has=!!box?.children?.length,force=memberForce46;memberForce46=false;if(!force&&has&&sig===lastMemberSig46)return;if(!force&&has&&memberIsMoving46()){clearTimeout(memberDeferred46);memberDeferred46=setTimeout(()=>{memberForce46=true;renderMembers()},190);return}memberRendering46=true;try{renderMembersPrev46Safe();lastMemberSig46=sig}finally{memberRendering46=false}};
...
(()=>{
window.__kokmatchVersionLock='6.33';
...
/* v6.21: obsolete v5.4 fix21~26 dynamic bridge removed; code is already consolidated below. */
...
function grade22(m){const c=String(m?.cls||'C').trim().toUpperCase(),safe=['S','A','B','C','D','E'].includes(c)?c:'C';return `<span class="tag gradeBadge50 grade-${safe.toLowerCase()}50">${e22(m?.age||'30')}${e22(safe)}</span>`}
...
function buildLabelV6(){return 'v6.20'}
async function forceLatestHeaderRefreshV6(){
 ...
 }finally{setTimeout(()=>{window.__kokmatchHeaderRefreshBusyV6=false;if(b){b.disabled=false;b.textContent='↻ 새로고침'}},1200)}
}
...
function v615StripPartnerNames(){document.querySelectorAll('#members .memberRelation83,#members .relationInfo66').forEach(el=>{const t=String(el.textContent||'').trim();el.classList.toggle('v616PartnerNameHidden',/^파트너(?:\s|$)/.test(t)||/·\s*파트너(?:\s|$)/.test(t))})}
...
function v615PartnerMeta(m){const y=String(m?.year||'').trim(),g=String(m?.gender||'').trim(),c=String(m?.cls||'').trim().toUpperCase();return [y?y+'년생':'출생연도 미입력',g||'성별 미입력',c?c+'급':'급수 미입력'].join(' · ')}
...
function repairMemberControlsV6(){
 // v6.85: obsolete roster rail rewriting disabled; canonical role-safe rail owns this DOM.
 return;
...
function canRoleFrameSetting681(){try{if(!me)return false;const direct=String(me.role||'');const id=String(me.memberId||me.id||'');const linked=Array.isArray(S?.members)?S.members.find(m=>String(m?.id||'')===id):null;const linkedRole=String(linked?.role||'');return !!me.globalAdmin||['admin','manager','organizer'].includes(direct)||['admin','manager','organizer'].includes(linkedRole)}catch{return false}}
...
function laterV6(){if(window.__kokmatchLaterV615)return;window.__kokmatchLaterV615=1;requestAnimationFrame(()=>{window.__kokmatchLaterV615=0;try{syncUiV6();v615StripPartnerNames()}catch{}})}
document.addEventListener('click',ev=>{if(Date.now()-lastTouchV6<750)return;const btn=ev.target?.closest?.('button');if(window.__kokmatchMemberPointerRouterV615&&btn?.closest?.('#members .memberCard,#memberEditorV615,#partnerOverlayV615'))return;if(routeButtonV6(btn)){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();laterV6()}},true);
window.addEventListener('touchstart',ev=>{if(ev.target?.closest?.('#modal,#memberEditorV615,#partnerOverlayV615,#members .memberCard'))return;const t=ev.touches?.[0];if(!t)return;sxV6=t.clientX;syV6=t.clientY;stV6=Date.now();movedV6=false},{capture:true,passive:true});
window.addEventListener('touchmove',ev=>{if(ev.target?.closest?.('#modal,#memberEditorV615,#partnerOverlayV615,#members .memberCard'))return;const t=ev.touches?.[0];if(!t)return;if(Math.abs(t.clientX-sxV6)>12||Math.abs(t.clientY-syV6)>12)movedV6=true},{capture:true,passive:true});
window.addEventListener('touchend',ev=>{if(ev.target?.closest?.('#modal,#memberEditorV615,#partnerOverlayV615,#members .memberCard'))return;const t=ev.changedTouches?.[0];if(!t||movedV6||Date.now()-stV6>900)return;let btn=ev.target?.closest?.('button');if(!btn&&typeof document.elementsFromPoint==='function'){for(const el of document.elementsFromPoint(t.clientX,t.clientY)){const b=el?.closest?.('button');if(b){btn=b;break}}}if(routeButtonV6(btn)){lastTouchV6=Date.now();ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation();laterV6()}},{capture:true,passive:false});
...
window.__kokmatchStandalone='6.33';
window.__kokmatchVersionLock='6.33';
...
window.__kokmatchMemberTemplateV618='v6.20';
...
function gradeBadgeV618(m){const c=['S','A','B','C','D','E'].includes(String(m?.cls||'').toUpperCase())?String(m.cls).toUpperCase():'C';return '<span class="tag grade-'+c.toLowerCase()+'50">'+escV618(m?.age||'30')+c+'</span>'}
...
function openMemberEditorV618(m){
 ...
 const add=!m,r=m?roleV618(m):'member',actor=(()=>{try{return me?.globalAdmin?'admin':String(me?.role||'member')}catch{return'member'}})(),isAdmin=!add&&r==='admin',roleEditable=!!me?.globalAdmin||actor==='manager',typeLocked=isAdmin||(actor==='organizer'&&!add&&r!=='member');
...
(()=>{
 ...
 if(Date.now()<Number(window.__kokmatchResumeNoRailReplaceUntil638||0))return;
  ...
  async function saveEditor(form){
    ...
    if(!['S','A','B','C','D','E'].includes(cls))return editorError('급수는 S/A/B/C/D/E 중에서 선택해주세요.');
    ...
    try{
      const x=await stableApi('member_save',{memberId:id,name,year,gender,cls,type,role,pin,inviter,compact:true});
      if(x?.data){
        try{S=x.data;window.S=x.data;normalizeClient()}catch{}
      }else if(x?.member){
...
function gradeTag623(m){const c=String(m?.cls||'C').trim().toUpperCase();return `<span class="pollGrade623${c==='S'?' grade-s50':''}">${esc(String(m?.age||'30'))}${esc(c)}</span>`}
...
function calendar623(){
 ...
 for(let d=1;d<=last;d++){const dt=date623(y,m,d),sel=dt===selectedDate623,isToday=dt===today623(),withPoll=has.has(dt),past=dt<today623();cells.push(`<button class="pollDay623 ${weekdayClass623(dt)} ${sel?'selected':''} ${isToday?'today':''} ${withPoll?'hasPoll':''} ${past?'past623':''}" onclick="selectPollDate623('${dt}')"><span>${d}</span></button>`)}
...
function renderPoll623(){const box=$('stats');if(!box)return;cleanupOldPollDom623();let wrap=box.querySelector('.pollWrap623');if(!wrap){wrap=document.createElement('div');wrap.className='pollWrap623'}if(box.firstElementChild!==wrap)box.prepend(wrap);wrap.innerHTML=section623()}
...
window.createPoll623=async function(){const v=readPollForm623();if(v.error)return alert(v.error);try{await requestPoll623('poll_create',v);closeModal();renderStats();goView('stats')}catch(e){showError(e)}};
window.savePollEdit623=async function(id){const v=readPollForm623();if(v.error)return alert(v.error);try{await requestPoll623('poll_update',{pollId:id,...v});closeModal();renderStats();goView('stats')}catch(e){showError(e)}};
window.deletePoll623=async function(id){const p=poll623(id);if(!p||!staff623())return;if(ended623(p)&&!permanentStaff623())return alert('종료된 투표는 모임장 또는 운영진만 삭제할 수 있습니다.');if(!confirm('이 참석 투표를 삭제하시겠습니까?'))return;try{await requestPoll623('poll_delete',{pollId:id});renderStats()}catch(e){showError(e)}};
...
/* KokMatch v6.33 independent date + compact monthly statistics */
(()=>{
'use strict';
const STATS628_API='https://wjelumpbjklfrdjxbesj.supabase.co/functions/v1/kokmatch-stats-v54';
const today628=()=>new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Seoul',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());
let statsDate628=today628(),statsMonth628=statsDate628.slice(0,7),statsData628=null,statsKey628='',statsSig628='',statsLoading628=false,statsError628='',statsSeq628=0,sortKey628='attendance',sortDir628='desc',zeroOnly628=false;

function esc628(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
function currentMonth628(){return today628().slice(0,7)}
function stateSig628(){try{return (S?.members||[]).map(m=>[m.id,m.totalGames,m.attendanceMonth,m.attendanceCount,m.attendanceHistory?.[statsMonth628]??''].join(':')).join('|')+'#'+String(S?.history?.length||0)}catch{return''}}
function dateTitle628(dt){const [y,m,d]=String(dt||'').split('-').map(Number);if(!y||!m||!d)return String(dt||'')+' 통계';return `${m}월 ${d}일 통계${dt===today628()?' · 오늘':''}`}
function fmt628(n){const v=Math.round((Number(n)||0)*10)/10;return Number.isInteger(v)?String(v):v.toFixed(1)}
function roleText628(r){r=String(r||'member');return r==='admin'?'개발자':r==='manager'?'모임장':r==='organizer'?'운영진':'일반'}
function roleRank628(r){r=String(r||'member');return r==='admin'?0:r==='manager'?1:r==='organizer'?2:3}
function monthShift628(ym,delta){const [y,m]=String(ym).split('-').map(Number),d=new Date(Date.UTC(y,m-1+Number(delta||0),1));return `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}`}
function dateShift628(dt,delta){const [y,m,d]=String(dt).split('-').map(Number),x=new Date(Date.UTC(y,m-1,d+Number(delta||0)));return `${x.getUTCFullYear()}-${String(x.getUTCMonth()+1).padStart(2,'0')}-${String(x.getUTCDate()).padStart(2,'0')}`}
function baselineMap628(data){return new Map((data?.baselines||[]).map(b=>[String(b.memberId||''),b]))}
function attendanceFor628(m,data){const hist=m?.attendanceHistory&&typeof m.attendanceHistory==='object'?m.attendanceHistory:{};const hv=Number(hist[statsMonth628]);if(Number.isFinite(hv))return Math.max(0,hv);if(String(m?.attendanceMonth||'')===statsMonth628)return Math.max(0,Number(m?.attendanceCount)||0);const b=baselineMap628(data).get(String(m?.id||''));return Math.max(0,Number(b?.attendanceCount)||0)}
function gamesFor628(m,data){const id=String(m?.id||''),games=(data?.monthGames||[]).filter(g=>(g?.players||[]).map(String).includes(id)),b=baselineMap628(data).get(id);if(!b)return games.length;const captured=Number(b.capturedAtMs)||0;return Math.max(0,Number(b.gameCount)||0)+games.filter(g=>(Number(g?.endedAt)||0)>captured).length}
function avgWaitMin628(games){const vals=[];for(const g of games||[])for(const v of Object.values(g?.waitMsByPlayer||{})){const n=Number(v);if(Number.isFinite(n)&&n>=0)vals.push(n)}if(!vals.length)return null;return vals.reduce((a,b)=>a+b,0)/vals.length/60000}
function rows628(){const data=statsData628||{};return (data.members||[]).filter(m=>String(m?.type||'member')!=='guest').map(m=>({id:String(m.id||''),name:String(m.name||''),year:Number(m.year)||0,age:String(m.age||''),cls:String(m.cls||''),role:String(m.role||'member'),attendance:attendanceFor628(m,data),games:gamesFor628(m,data)}))}
function compare628(a,b){let n=0;if(sortKey628==='name')n=a.name.localeCompare(b.name,'ko');else if(sortKey628==='year')n=a.year-b.year;else if(sortKey628==='agegrade')n=(Number(a.age)||0)-(Number(b.age)||0)||a.cls.localeCompare(b.cls,'ko');else if(sortKey628==='role')n=roleRank628(a.role)-roleRank628(b.role)||a.name.localeCompare(b.name,'ko');else if(sortKey628==='games')n=a.games-b.games;else n=a.attendance-b.attendance;if(n===0&&sortKey628!=='name')n=a.name.localeCompare(b.name,'ko');return sortDir628==='asc'?n:-n}
function arrow628(key){return sortKey628===key?(sortDir628==='asc'?'▲':'▼'):''}
function renameStatsNav628(){const b=document.querySelector('nav button[data-v="stats"]');if(b)b.innerHTML='<i>▥</i>운동통계'}

function statsHtml628(){
 const data=statsData628,loading=statsLoading628&&!data,error=statsError628,daily=data?.rangeGames||[],mins=daily.reduce((a,g)=>a+(Number(g?.durationMin)||0),0),people=new Set(daily.flatMap(g=>(g?.players||[]).map(String))).size,avgWait=avgWaitMin628(daily);
 const all=rows628(),zeroCount=all.filter(r=>r.attendance===0).length,shown=(zeroOnly628?all.filter(r=>r.attendance===0):all).slice().sort(compare628),monthLabel=statsMonth628.replace('-','년 ')+'월';
 const rows=shown.map(r=>`<tr data-member-id628="${esc628(r.id)}"><td title="${esc628(r.name)}"><b>${esc628(r.name)}</b></td><td>${r.year||'-'}</td><td>${esc628((r.age||'-')+(r.cls||''))}</td><td><span class="statsRole628 role-${esc628(r.role)}">${roleText628(r.role)}</span></td><td><b>${r.attendance}</b></td><td><b>${r.games}</b></td></tr>`).join('');
 return `<section class="statsDate628"><div class="statsDateHead628"><div><b>날짜별 통계</b><span class="statsDateTitle628" data-date="${esc628(statsDate628)}">${esc628(dateTitle628(statsDate628))}</span></div><div class="statsDatePicker628"><button type="button" onclick="moveStatsDate628(-1)" aria-label="이전 날짜">‹</button><input id="statsDateInput628" type="date" value="${esc628(statsDate628)}" max="${esc628(today628())}" onchange="changeStatsDate628(this.value)"><button type="button" ${statsDate628>=today628()?'disabled':''} onclick="moveStatsDate628(1)" aria-label="다음 날짜">›</button></div></div>${loading?'<div class="statsLoading628">통계를 불러오는 중...</div>':error?`<div class="warn statsError628">${esc628(error)} <button class="miniBtn" onclick="reloadStats628()">다시 불러오기</button></div>`:`<div class="statsGrid statsDateGrid628"><div class="stat"><b>${daily.length}</b><span>완료 게임</span></div><div class="stat"><b>${fmt628(mins)}분</b><span>플레이시간</span></div><div class="stat"><b>${people}</b><span>참여 인원</span></div><div class="stat"><b>${avgWait==null?'-':fmt628(avgWait)+'분'}</b><span>평균 게임대기</span></div></div>`}</section><section class="statsMonthly628"><div class="statsMonthlyHead628"><div><b>월간 회원 기록</b><span>${esc628(monthLabel)} · 현재 등록 회원 기준</span></div><div class="statsMonthPicker628"><button type="button" onclick="moveStatsMonth628(-1)" aria-label="이전 달">‹</button><input id="statsMonthInput628" type="month" value="${esc628(statsMonth628)}" max="${esc628(currentMonth628())}" onchange="changeStatsMonth628(this.value)"><button type="button" ${statsMonth628>=currentMonth628()?'disabled':''} onclick="moveStatsMonth628(1)" aria-label="다음 달">›</button></div></div><div class="statsFilter628"><button type="button" class="${zeroOnly628?'':'on'}" onclick="setZeroAttendance628(false)">전체 ${all.length}명</button><button id="statsZeroOnly628" type="button" class="${zeroOnly628?'on':''}" onclick="setZeroAttendance628(true)">출석 0회 ${zeroCount}명</button></div>${loading?'<div class="statsLoading628">월간 기록을 불러오는 중...</div>':error?`<div class="warn statsError628">${esc628(error)}</div>`:`<div class="statsTableWrap628"><table class="statsMonthlyTable628"><colgroup><col class="cName628"><col class="cYear628"><col class="cAge628"><col class="cRole628"><col class="cAttend628"><col class="cGames628"></colgroup><thead><tr><th><button data-stats-sort628="name" onclick="sortMonthly628('name')">이름${arrow628('name')}</button></th><th><button data-stats-sort628="year" onclick="sortMonthly628('year')">년생${arrow628('year')}</button></th><th><button data-stats-sort628="agegrade" onclick="sortMonthly628('agegrade')">나이급수${arrow628('agegrade')}</button></th><th><button data-stats-sort628="role" onclick="sortMonthly628('role')">역할${arrow628('role')}</button></th><th><button data-stats-sort628="attendance" onclick="sortMonthly628('attendance')">출석${arrow628('attendance')}</button></th><th><button data-stats-sort628="games" onclick="sortMonthly628('games')">게임${arrow628('games')}</button></th></tr></thead><tbody>${rows||`<tr><td colspan="6" class="statsEmpty628">${zeroOnly628?'선택한 달에 출석 0회인 회원이 없습니다.':'표시할 회원 기록이 없습니다.'}</td></tr>`}</tbody></table></div>`}</section>`;
}
function renderStatsDetail628(){const box=document.getElementById('stats');if(!box)return;renameStatsNav628();let detail=box.querySelector('.statsDetail628');if(!detail){detail=document.createElement('div');detail.className='statsDetail628';box.appendChild(detail)}[...box.children].forEach(el=>{if(el!==detail&&!el.classList.contains('pollWrap623')&&el.id!=='opsDashboard652')el.remove()});const poll=box.querySelector('.pollWrap623');if(poll&&box.firstElementChild!==poll)box.prepend(poll);if(poll&&poll.nextElementSibling!==detail)poll.after(detail);detail.innerHTML=statsHtml628()}
async function loadStats628(force=false){if(!me||!currentGroupId)return;const key=`${currentGroupId}|${statsDate628}|${statsMonth628}`,sig=stateSig628();if(!force&&statsData628&&statsKey628===key&&statsSig628===sig){renderStatsDetail628();return}const seq=++statsSeq628;statsLoading628=true;statsError628='';renderStatsDetail628();try{const u=new URL(STATS628_API);u.searchParams.set('groupId',currentGroupId);u.searchParams.set('from',statsDate628);u.searchParams.set('to',statsDate628);u.searchParams.set('month',statsMonth628);const r=await fetch(u,{headers:{authorization:'Bearer '+T},cache:'no-store'}),x=await r.json().catch(()=>({}));if(!r.ok){if(r.status===401){try{reloginLatest()}catch{};throw new Error('로그인이 만료되었습니다.')}throw new Error(x.error||'통계를 불러오지 못했습니다.')}if(seq!==statsSeq628)return;statsData628=x;statsKey628=key;statsSig628=sig;statsError628=''}catch(e){if(seq!==statsSeq628)return;statsError628=e?.message||String(e||'통계를 불러오지 못했습니다.')}finally{if(seq===statsSeq628){statsLoading628=false;renderStatsDetail628()}}}
function mountStats628(){renderStatsDetail628();loadStats628(false)}
window.changeStatsDate628=function(dt){dt=String(dt||'');if(!/^\d{4}-\d{2}-\d{2}$/.test(dt)||dt>today628())return;statsDate628=dt;statsKey628='';window.__qaLastStatsDate=dt;mountStats628()};
window.moveStatsDate628=function(delta){const next=dateShift628(statsDate628,delta);if(next>today628())return;changeStatsDate628(next)};
window.changeStatsMonth628=function(mon){mon=String(mon||'');if(!/^\d{4}-\d{2}$/.test(mon)||mon>currentMonth628())return;statsMonth628=mon;zeroOnly628=false;statsKey628='';mountStats628()};
window.moveStatsMonth628=function(delta){const next=monthShift628(statsMonth628,delta);if(next>currentMonth628())return;changeStatsMonth628(next)};
window.setZeroAttendance628=function(on){zeroOnly628=!!on;renderStatsDetail628()};
window.sortMonthly628=function(key){if(!['name','year','agegrade','role','attendance','games'].includes(key))return;if(sortKey628===key)sortDir628=sortDir628==='asc'?'desc':'asc';else{sortKey628=key;sortDir628='asc'}renderStatsDetail628()};
window.reloadStats628=function(){statsKey628='';loadStats628(true)};
const renderStatsBefore628=renderStats;renderStats=function(...args){const r=renderStatsBefore628.apply(this,args);mountStats628();return r};window.renderStats=renderStats;
const goViewBefore628=goView;goView=function(id){const r=goViewBefore628(id);if(id==='stats')mountStats628();return r};window.goView=goView;
if(me&&currentView==='stats')mountStats628();
})();



/* KokMatch v6.33 PWA install + notification permission guide */
(()=>{
'use strict';
...
/* KokMatch v6.33 first-login session stabilization */
(()=>{
'use strict';
...
/* KokMatch v6.33: restore roster controls after iOS/Kakao background resume */
(()=>{
'use strict';
...
function paint632(){
 ...
 for(const card of box.querySelectorAll('.memberCard')){
  ...
  const rails632=[...card.querySelectorAll(':scope > .kmRosterActions621,:scope > .v6MemberActions,:scope > .memberActions48,:scope > .memberActions60,:scope > .memberActions65')];
...
async function repair632(reason='resume'){
 ...
 try{
  ...
  console.warn('KokMatch v6.33 roster resume repair',reason,e);
...
/* v6.34: resume state refresh is centralized below; painter632 remains available. */
...
const CURRENT633='6.85';
let checking633=false,reloading633=false,rosterTimer633=0,lastVersionCheck633=0;
...
function patchVersion633(){try{window.__kokmatchRenderGlobalVersion634?.()}catch{}}
...
async function checkVersion633(reason='startup'){if(checking633||reloading633)return false;const now633=Date.now();if(lastVersionCheck633&&now633-lastVersionCheck633<30000){patchVersion633();return false}lastVersionCheck633=now633;checking633=true;try{await ensureLatestSw633();const latest=await latest633(),cur=runtime633();patchVersion633();if(latest&&cmp633(latest,cur)>0){const key='kokmatch_update_reload_'+latest;let last=0;try{last=Number(sessionStorage.getItem(key)||0)}catch{}if(Date.now()-last>5000){try{sessionStorage.setItem(key,String(Date.now()))}catch{};await hardReload633(latest,reason);return true}}return false}finally{checking633=false}}
try{forceUpdateApp=async function(){const b=document.getElementById('forceUpdateBtn');if(b){b.disabled=true;b.textContent='최신 운영본 확인 중...'}try{const latest=await latest633();await hardReload633(latest||CURRENT633,'manual')}catch(e){if(b){b.disabled=false;b.textContent='↻ 새로고침'};try{showError(e)}catch{alert(e?.message||String(e))}}};window.forceUpdateApp=forceUpdateApp}catch{}
...
const BUILD634=Object.freeze({version:'6.85',label:'콕매치 v6.85 · 최신 운영본'});
...
function renderGlobalVersion634(){
 ...
 try{
  ...
  if(!card){
   ...
   card.innerHTML='<div class="between"><div><b>프로그램 버전</b><div id="kokmatchGlobalVersion634" class="meta"></div></div><span class="tag">운영본</span></div><button id="forceUpdateBtn" class="btn pri" style="width:100%;margin-top:10px" onclick="forceUpdateApp()">↻ 새로고침</button>';
  ...
  if(!card.querySelector('#forceUpdateBtn')){const b=document.createElement('button');b.id='forceUpdateBtn';b.className='btn pri';b.style.cssText='width:100%;margin-top:10px';b.textContent='↻ 새로고침';b.onclick=()=>forceUpdateApp();card.appendChild(b)}
...
/* KokMatch v6.85: role-safe canonical member paging */
(()=>{
'use strict';
...
try{memberControls=controlsHtml637;window.memberControls=memberControls}catch(e){console.warn('KokMatch v6.85 memberControls source',e)}
...
function stabilize637(force=false){
 if(!force&&Date.now()<Number(window.__kokmatchResumeNoRailReplaceUntil638||0)&&!needs637())return;
...
try{
 ...
 renderMembers=function(...args){const r=baseRender637.apply(this,args);stabilize637(true);schedule637(false);return r};
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.85 renderMembers wrapper',e)}
try{
...
}catch(e){console.warn('KokMatch v6.85 member pager wrapper',e)}
function arm637(){
...
/* KokMatch v6.85: no-flash member roster resume */
(()=>{
'use strict';
...
/* KokMatch v6.85: preflight + atomic conflict recovery for multi-organizer game creation */
(()=>{
'use strict';
...
/* KokMatch v6.85: white member/queue cards + right-side queue grade stripe. */
(()=>{
'use strict';
...
/* KokMatch v6.85: personal queue grade watermark only. No card layout mutation. */
(()=>{
'use strict';
...
/* KokMatch v6.85: live operations dashboard inside 운동통계. */
(()=>{
'use strict';
...
function dashboardHtml652(){
 ...
 return `<section id="opsDashboard652" class="opsDashboard652">
  <div class="opsHead652"><div><div class="opsEyebrow652">LIVE OPERATIONS</div><h2>실시간 운영현황</h2><p>${esc652(group?.name||'현재 모임')} · 운영상태 자동동기화 · 전체 회원명부 보존</p></div><div class="opsState652 ${st.cls}"><i></i>${st.label}<small>${now} 기준</small></div></div>
...
function renderOpsDashboard652(){
 ...
 if(old){old.className=next.className;old.innerHTML=next.innerHTML}else box.prepend(next);
}
...
/* v6.85: group-scoped AI auto game assignment + pending creator provenance. */
(()=>{
'use strict';
if(window.__kokmatchAutoGame656)return;
window.__kokmatchAutoGame656='6.85';
...
function autoCfg656(){const c=S?.autoGame&&typeof S.autoGame==='object'?S.autoGame:{};return{enabled:c.enabled===true,mode:'priority_v685',updatedAt:Number(c.updatedAt)||0,updatedBy:c.updatedBy&&typeof c.updatedBy==='object'?c.updatedBy:{}}}
...
function autoRuleHtml656(){return `<div class="autoRules656">
 ...
 <div><b>④ 최근 반복조합 최소화</b><span>최근 1시간 안에 함께 경기한 조합은 가능한 한 다시 묶이지 않도록 먼저 분산합니다. 오늘 파트너 조합은 예외입니다.</span></div>
 <div><b>⑤ 급수·남녀 밸런스</b><span>최근 반복을 줄인 범위 안에서 팀 전력과 남녀 구성을 맞춥니다. 전력 계산에서는 여자 C조를 남자 D조와 같은 수준으로 환산합니다.</span></div>
 <div><b>⑥ 당일 누적 반복 최소화</b><span>당일 3회 이상 같이 경기한 조합과 누적 반복횟수를 추가로 줄입니다.</span></div>
 </div>`}
window.openAutoGameSettings656=function(){
 ...
 openModal(`<h3>자동게임설정 · 우선순위 자동편성</h3><div class="note">아래 순서를 기준으로 자동 편성합니다. 자동편성은 개인 게임대기 인원이 4명 미만이 될 때까지 계속 조를 만들되, 곧 끝날 경기 인원을 섞는 편이 더 좋은 경우에는 마지막 인원을 잠시 기다립니다.</div>${autoRuleHtml656()}<div class="autoPolicy656"><b>운영 안전장치</b><span>1~3명짜리 미완성 대기조가 있으면 자동편성은 새 조를 만들지 않고 잠시 멈춥니다. 기존 대기조 인원을 임의로 바꾸거나 해체하지 않습니다.</span><span>코트 수와 관계없이 개인 게임대기가 4명 이상이면 4명 미만이 남을 때까지 순차적으로 자동 편성합니다.</span><span>진행 중 경기의 종료 예상이 5분 이내이고 개인대기가 마지막 4~7명만 남은 경우에는 그 인원을 바로 묶지 않고 잠시 대기시킵니다. 경기가 끝난 4명이 합류하면 다시 전체 후보를 섞어 최근 반복조합을 줄여 편성합니다.</span><span>자동편성 ON 상태에서 개발자·모임장·운영진이 수동 편성을 시도하면 먼저 동작을 막고, 자동기능 유지 또는 자동기능을 끄고 직접 편성 중 하나를 선택하게 합니다.</span></div>${u.name?`<div class="meta autoUpdated656">현재 설정자: <b>${escAuto656(u.name)}</b> · ${escAuto656(u.roleLabel||roleText656(u.role))}</div>`:''}<button class="btn ghost" style="width:100%;margin-top:12px" onclick="closeModal()">닫기</button>`);
};
...
function autoCardHtml656(){
 ...
 return `<div class="card autoGameCard656 ${on?'on':'off'}"><div class="autoGameTop656"><div><div class="autoGameTitle656"><b>자동게임편성</b><span class="autoPill656">AI 자동 최적화</span></div><div class="meta">개발자 · 모임장 · 운영진 공용 설정</div></div><button type="button" class="autoToggle656 ${on?'on':''}" onclick="toggleAutoGame656()" ${autoToggleBusy656?'disabled':''}><span>${on?'ON':'OFF'}</span><i></i></button></div><div class="autoGameDesc656">${on?'개인대기 4명 이상이면 4명 미만이 남을 때까지 연속 편성하고, 종료 임박 경기 인원을 섞는 편이 유리하면 마지막 4~7명은 잠시 기다립니다.':'현재는 수동 편성입니다. ON으로 바꾸면 AI 자동편성이 시작됩니다.'}</div>${u.name?`<div class="meta autoBy656">마지막 설정 · ${escAuto656(u.name)} · ${escAuto656(u.roleLabel||roleText656(u.role))}</div>`:''}<button type="button" class="btn ghost autoSettingsBtn656" onclick="openAutoGameSettings656()">자동게임설정</button></div>`;
}
...
renderSettings=function(...args){const r=renderSettingsBefore656.apply(this,args);paintAutoSettings656();const box=document.getElementById('settings');if(box)[...box.querySelectorAll('.meta')].forEach(el=>{if(/콕매치 v6\.5[0-5]/.test(el.textContent||''))el.textContent='콕매치 v6.85 · AI 자동게임편성 · 편성자 기록'});return r};
...
async function runAuto656(force=false){
 if(!T||!currentGroupId||document.hidden||!autoEnabled656()||!canControlAuto656())return null;
 ...
 try{
  ...
  return x;
 }catch(e){console.warn('auto game v6.85',e);return null}finally{autoBusy656=false}
}
...
/* v6.85 keep today live dashboard mounted */
setTimeout(()=>{if(me){paintAutoSettings656();decorateQueueAuto656()}armAuto656()},0);
})();


/* v6.85: waiting-screen auto controls, clean member badges, developer challenger frame, queue meta alignment. */
(()=>{
'use strict';
if(window.__kokmatchUiRefine658)return;
window.__kokmatchUiRefine658='6.85';
...
/* v6.85 final ordinary-badge, developer-frame and personal queue metadata guard. */
(()=>{
'use strict';
...
/* v6.85 compact auto control + Challenger crest profile frame. */
(()=>{
'use strict';
window.__kokmatchUiRefine659='6.85';
let frameQueued659=false;

function canAuto659(){try{const linked=me?.memberId&&typeof M==='function'?M(String(me.memberId)):null;const roles=[String(me?.role||''),String(linked?.role||'')];return !!me&&(me.globalAdmin===true||roles.some(r=>r==='admin'||r==='manager'||r==='organizer'))}catch{return !!me&&(me.globalAdmin===true||me.role==='manager'||me.role==='organizer')}}
...
window.__kokmatchDevFrame661='6.85';
...
/* v6.85 final roster-card cleanup: no rectangular border/accent survives legacy render styles. */
(()=>{
 'use strict';
...
/* v6.85: manager gold aura + organizer silver aura */
(()=>{
'use strict';
if(window.__kokmatchRoleAura664)return;
window.__kokmatchRoleAura664='6.85';
const MANAGER_SRC='/assets/dev-prism-frame-v662.webp?v=6.85';
const ORGANIZER_SRC='/assets/organizer-silver-aura-v665.webp?v=6.85';
let queued=false;
function role664(id){
...
function ensure664(target,role){
 ...
 const src=role==='manager'?MANAGER_SRC:ORGANIZER_SRC;
...
/* v6.85: automatic mode owns game composition until an authorized operator explicitly switches to manual. */
(()=>{
'use strict';
if(window.__kokmatchAutoManualGuard673)return;window.__kokmatchAutoManualGuard673='6.85';
...
/* v6.85: canonical always-visible developer badge + restored Challenger frame. */
(()=>{
'use strict';
if(window.__kokmatchDeveloperVisible674)return;window.__kokmatchDeveloperVisible674='6.85';
const DEV_FRAME674='/assets/dev-prism-frame-v662.webp?v=6.85';
...
/* v6.85: verified automatic-game controls + stable stats polish. */
(()=>{
'use strict';
if(window.__kokmatchFix675)return;window.__kokmatchFix675='6.85';
...
window.openAutoGameSettings656=function(...args){
 ...
 syncAuto675().catch(e=>console.warn('auto settings sync v6.85',e));
 return r;
};
window.toggleAutoGame656=async function(){
...
/* v6.85: resilient automatic-game toggle; save state and composition execution are separate. */
(()=>{
'use strict';
if(window.__kokmatchAutoToggle676)return;window.__kokmatchAutoToggle676='6.85';
...
window.toggleAutoGame656=async function(){
 ...
 try{
  try{const before=await req676('get');apply676(before);serverBefore=before?.config?.enabled===true}catch(e){console.warn('auto pre-sync v6.85',e)}
  ...
  req676('get').then(v=>{apply676(v);paint676()}).catch(e=>console.warn('auto verify v6.85',e));
  ...
  if(next){
   try{
    ...
    console.warn('auto initial tick v6.85',e);
...
window.openAutoGameSettings656=function(...args){
 ...
 sync676().catch(e=>console.warn('auto settings sync v6.85',e));
 return r;
};
...
/* v6.85: roster no-flash, restored green queue game badge, persistent auto/manual conflict modal. */
(()=>{
'use strict';
if(window.__kokmatchUiStability677)return;window.__kokmatchUiStability677='6.85';
...
function memberStructure677(){
 try{
  ...
  return JSON.stringify([String(currentGroupId||''),String(me?.role||''),!!me?.globalAdmin,!!me?.tempOrganizer,
   '',Number(window.__kokmatchMemberPageRequested46||window.__kokmatchMemberPage46||1),members]);
 }catch{return String(Date.now())}
}
...
/* v6.85: make legacy roster finalizer idempotent so operational polling cannot recreate avatars. */
(()=>{
'use strict';
if(window.__kokmatchRosterFinalizerStable677)return;
const original=window.__kokmatchFinalizeRoster22;
if(typeof original!=='function')return;
window.__kokmatchRosterFinalizerStable677='6.85';
let last='';
function sig677(){
 try{
...
/* v6.85: final member-view renderAll gate. Legacy v22 calls finalizeRoster22 directly, so bypass that chain for operational-only updates. */
(()=>{
'use strict';
if(window.__kokmatchMemberRenderGate677)return;
window.__kokmatchMemberRenderGate677='6.85';
...
/* v6.85: persistent live-operations shell, permanent queue game badge, inline monthly rank circles, mobile roster spacing. */
(()=>{
'use strict';
if(window.__kokmatchUiStability678)return;window.__kokmatchUiStability678='6.85';
...
goView=function(id,...args){const r=goViewBefore678.call(this,id,...args);ensureOps678();if(id==='queue')window.__kokmatchRefreshPendingEtas680?.();setTimeout(ensureOps678,0);return r};window.goView=goView;
...
setInterval(()=>{if(me&&currentView==='queue'){canonicalQueue678();window.__kokmatchRefreshPendingEtas680?.()}},15000);
document.addEventListener('visibilitychange',()=>window.__kokmatchRefreshPendingEtas680?.());
...
/* v6.85: visible-shell resilience after PWA/service-worker updates. */
(()=>{
'use strict';
if(window.__kokmatchBootResilience679)return;window.__kokmatchBootResilience679='6.85';
...
function ensureVisibleShell679(){
 ...
 }catch(e){console.warn('KokMatch v6.85 visible shell recovery',e)}
 return false;
}
...
/* v6.85 final court-count focus/draft guard. */
(()=>{
'use strict';
if(window.__kokmatchCourtInputGuard681)return;window.__kokmatchCourtInputGuard681='6.85';
...
try{const baseSettings681=renderSettings;renderSettings=function(...args){if(protect681())return;return baseSettings681.apply(this,args)};window.renderSettings=renderSettings}catch(e){console.warn('v6.85 settings focus guard',e)}
try{const baseAll681=renderAll;renderAll=function(...args){const active=document.activeElement,keep=currentView==='settings'&&active?.id==='courtCountInput';if(!keep)return baseAll681.apply(this,args);const value=active.value,start=active.selectionStart,end=active.selectionEnd,y=window.scrollY;const r=baseAll681.apply(this,args);restore681(value,start,end,y);queueMicrotask(()=>{if(document.activeElement?.id!=='courtCountInput')restore681(value,start,end,y)});return r};window.renderAll=renderAll}catch(e){console.warn('v6.85 court renderAll guard',e)}
})();


/* v6.85: profile-photo reconciliation + member roster distribution dashboard. */
(()=>{
'use strict';
if(window.__kokmatchRosterProfileInsights682)return;
window.__kokmatchRosterProfileInsights682='6.85';

function memberId682(card){
 return String(card?.dataset?.memberId22||card?.dataset?.memberId||card?.dataset?.memberId46||card?.dataset?.memberId80||'');
}
function avatarNode682(m){
 try{
  if(!m||typeof avatar!=='function')return null;
  const holder=document.createElement('div');
  holder.innerHTML=String(avatar(m)||'').trim();
  const node=holder.firstElementChild;
  if(!node)return null;
  const id=String(m.id||'');
  if(id){
   node.dataset.memberId=id;
   node.dataset.memberId22=id;
  }
  return node;
 }catch{return null}
}
function currentAvatar682(card){
 if(!card)return null;
 return [...card.children].find(el=>el?.classList?.contains('avatar'))||
  card.querySelector(':scope > .profileIdentity21,:scope > .profileAvatar53,:scope > .avatar,:scope > .profileIdentity80,:scope > .genderAvatar39');
}
function photoSrc682(el){
 if(!el)return '';
 const imgs=[...el.querySelectorAll(':scope > img')];
 const img=imgs.find(x=>!x.classList.contains('devFrame661')&&!x.classList.contains('roleAura664'));
 return String(img?.getAttribute('src')||'');
}
function avatarSig682(el){
 if(!el)return '';
 return [
  String(el.getAttribute('data-photo')||''),
  String(el.getAttribute('data-gender')||''),
  photoSrc682(el),
  el.classList.contains('profileIdentity21')?'p21':'',
  el.classList.contains('hasPhoto21')?'photo':'',
  el.classList.contains('noPhoto21')?'fallback':''
 ].join('|');
}
let syncingProfiles682=false;
function syncRosterProfiles682(){
 if(syncingProfiles682)return false;
 const box=document.getElementById('members');
 if(!box||!Array.isArray(S?.members))return false;
 syncingProfiles682=true;
 let changed=false;
 try{
  const map=new Map(S.members.map(m=>[String(m?.id||''),m]));
  const cards=[...box.querySelectorAll('.memberCard')];
  cards.forEach((card,index)=>{
   const id=memberId682(card);
   const m=map.get(id)||S.members[index]||null;
   if(!m)return;
   const next=avatarNode682(m);
   if(!next)return;
   const cur=currentAvatar682(card);
   if(!cur||avatarSig682(cur)!==avatarSig682(next)){
    if(cur)cur.replaceWith(next);else card.insertBefore(next,card.firstChild);
    changed=true;
   }else{
    cur.dataset.memberId=String(m.id||'');
    cur.dataset.memberId22=String(m.id||'');
   }
  });
 }finally{syncingProfiles682=false}
 if(changed){
  queueMicrotask(()=>{
   try{window.__kokmatchApplyChallenger659?.()}catch{}
   try{window.__kokmatchSyncRoleAura664?.()}catch{}
   try{window.__kokmatchSyncDeveloper674?.()}catch{}
  });
 }
 return changed;
}
window.__kokmatchSyncRosterProfiles682=syncRosterProfiles682;

const gradeOrder682=['S','A','B','C','D','E'];
const gradeColor682={S:'#9a8df4',A:'#ef8d7d',B:'#f1ad61',C:'#f2cf58',D:'#66d39a',E:'#76aaf2'};
function pct682(n,total){return total?Math.round((Number(n)||0)*100/total):0}
function ageBand682(m){
 let decade=Number(String(m?.age||'').replace(/[^\d]/g,''));
 const y=Number(m?.year)||0;
 if(y>=1900){
  const now=new Date();
  const age=Math.max(0,now.getFullYear()-y);
  decade=Math.floor(age/10)*10;
 }
 if(!Number.isFinite(decade)||decade<=0)decade=30;
 if(decade<30)return 20;
 if(decade<40)return 30;
 if(decade<50)return 40;
 if(decade<60)return 50;
 return 60;
}
function donutBackground682(entries,total){
 if(!total)return '#edf1f6';
 let at=0,parts=[];
 for(const x of entries){
  if(!x.count)continue;
  const from=at;
  at+=x.count*100/total;
  parts.push((gradeColor682[x.grade]||'#9aa6b2')+' '+from.toFixed(2)+'% '+at.toFixed(2)+'%');
 }
 return parts.length?'conic-gradient('+parts.join(',')+')':'#edf1f6';
}
function paintMemberInsights682(){
 const box=document.getElementById('members');
 if(!box||!Array.isArray(S?.members))return;
 const members=S.members.filter(Boolean);
 const total=members.length;
 let panel=document.getElementById('memberInsights682');
 if(!panel){
  panel=document.createElement('section');
  panel.id='memberInsights682';
  panel.setAttribute('aria-label','모임 회원 분포');
  const note=box.querySelector(':scope > .note');
  const title=box.querySelector(':scope > .title');
  if(note)note.insertAdjacentElement('afterend',panel);
  else if(title)title.insertAdjacentElement('afterend',panel);
  else box.prepend(panel);
 }
 const gradeCounts=Object.fromEntries(gradeOrder682.map(g=>[g,0]));
 for(const m of members){
  const g=String(m?.cls||'C').trim().toUpperCase();
  gradeCounts[gradeOrder682.includes(g)?g:'C']++;
 }
 const grades=gradeOrder682.map(grade=>({grade,count:gradeCounts[grade]}));
 const ages={20:0,30:0,40:0,50:0,60:0};
 members.forEach(m=>ages[ageBand682(m)]++);
 const ageEntries=[20,30,40,50,60].map(age=>({age,count:ages[age]}));
 const ageMax=Math.max(1,...ageEntries.map(x=>x.count));
 const female=members.filter(m=>String(m?.gender||'').trim()==='여').length;
 const male=Math.max(0,total-female);
 const mp=pct682(male,total),fp=total?100-mp:0;
 const donut=donutBackground682(grades,total);

 panel.innerHTML=
  '<div class="memberInsightHeader682"><b>모임 정보</b><span>회원 '+total+'명</span></div>'+
  '<div class="memberInsightCard682 memberGradeCard682">'+
   '<div class="memberInsightCardTitle682">급수 분포</div>'+
   '<div class="memberGradeGrid682">'+
    '<div class="memberDonut682" style="background:'+donut+'"><div class="memberDonutCenter682"><strong>'+total+'명</strong><span>전체 회원</span></div></div>'+
    '<div class="memberGradeLegend682">'+grades.map(x=>'<div><i style="background:'+(gradeColor682[x.grade]||'#9aa6b2')+'"></i><span>'+x.grade+'급</span><b>'+x.count+'명</b><em>'+pct682(x.count,total)+'%</em></div>').join('')+'</div>'+
   '</div>'+
  '</div>'+
  '<div class="memberInsightCard682 memberAgeCard682">'+
   '<div class="memberInsightCardTitle682">연령 분포</div>'+
   '<div class="memberAgeBars682">'+ageEntries.map(x=>'<div class="memberAgeItem682" title="'+(x.age===60?'60대 이상':x.age+'대')+' '+x.count+'명"><div class="memberAgeCount682">'+x.count+'</div><div class="memberAgeTrack682"><i style="height:'+(x.count?Math.max(8,Math.round(x.count*100/ageMax)):0)+'%"></i></div><span>'+(x.age===60?'60대+':x.age+'대')+'</span></div>').join('')+'</div>'+
  '</div>'+
  '<div class="memberInsightCard682 memberGenderCard682">'+
   '<div class="memberInsightCardTitle682">성비</div>'+
   '<div class="memberGenderTrack682"><i class="male682" style="width:'+mp+'%"></i><i class="female682" style="width:'+fp+'%"></i></div>'+
   '<div class="memberGenderLegend682"><span><i class="male682"></i>남성 '+mp+'% <b>'+male+'명</b></span><span><i class="female682"></i>여성 '+fp+'% <b>'+female+'명</b></span></div>'+
  '</div>';
}
window.__kokmatchPaintMemberInsights682=paintMemberInsights682;

let profileTimer682=0;
function scheduleProfiles682(){
 clearTimeout(profileTimer682);
 profileTimer682=setTimeout(()=>{
  syncRosterProfiles682();
  paintMemberInsights682();
  requestAnimationFrame(()=>syncRosterProfiles682());
 },0);
}
function relevantMutation682(m){
 const target=m.target instanceof Element?m.target:m.target?.parentElement;
 if(target?.closest?.('#members .memberCard'))return true;
 for(const n of m.addedNodes||[]){
  if(!(n instanceof Element))continue;
  if(n.matches?.('.memberCard,.avatar,.profileIdentity21,.profileAvatar53')||n.querySelector?.('.memberCard,.avatar,.profileIdentity21,.profileAvatar53'))return true;
 }
 return false;
}

try{
 const prevMembers682=renderMembers;
 renderMembers=function(...args){
  const r=prevMembers682.apply(this,args);
  syncRosterProfiles682();
  paintMemberInsights682();
  queueMicrotask(syncRosterProfiles682);
  requestAnimationFrame(syncRosterProfiles682);
  return r;
 };
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.85 member render patch',e)}

try{
 const prevAll682=renderAll;
 renderAll=function(...args){
  const r=prevAll682.apply(this,args);
  if(currentView==='members')scheduleProfiles682();
  return r;
 };
 window.renderAll=renderAll;
}catch(e){console.warn('KokMatch v6.85 renderAll patch',e)}

try{
 const prevGo682=goView;
 goView=function(id,...args){
  const r=prevGo682.call(this,id,...args);
  if(id==='members')scheduleProfiles682();
  return r;
 };
 window.goView=goView;
}catch(e){console.warn('KokMatch v6.85 goView patch',e)}

function arm682(){
 const box=document.getElementById('members');
 if(!box||box.dataset.profileObserver682==='1')return;
 box.dataset.profileObserver682='1';
 new MutationObserver(ms=>{if(ms.some(relevantMutation682))scheduleProfiles682()}).observe(box,{childList:true,subtree:true});
 scheduleProfiles682();
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',arm682,{once:true});else arm682();
})();



/* v6.85: queue-screen cleanup, fixed bottom navigation guard, and concise pending cards. */
(()=>{
'use strict';
if(window.__kokmatchQueueUx683)return;
window.__kokmatchQueueUx683='6.85';

let cleaning683=false,cleanTimer683=0,observer683=null;

function personalRule683(box){
 try{
  const head=[...box.querySelectorAll(':scope > .subhead')].find(x=>String(x.textContent||'').includes('개인 게임대기'));
  const note=head?.nextElementSibling;
  if(note?.classList?.contains('note')){
   note.classList.add('queueRule683');
   if(String(note.textContent||'').trim()!=='게임 적은 순 · 대기 긴 순')note.textContent='게임 적은 순 · 대기 긴 순';
  }
 }catch{}
}

function simplifyPending683(box){
 try{
  const cards=[...box.querySelectorAll('.pendingCard:not(.composer54):not(.composer)')];
  for(const card of cards){
   /* Requested behavior: remove only the repeat/pair summary line from completed pending groups.
      Keep gender icon, age/grade, role badge, game count, wait time and guest inviter exactly as rendered. */
   card.classList.remove('pendingSimple683');
   card.classList.add('pendingPairOnly684');
   card.querySelectorAll(':scope > .pairSummary').forEach(el=>el.remove());
   card.querySelectorAll('.pendingPersonSimple683').forEach(slot=>slot.classList.remove('pendingPersonSimple683'));
   card.querySelectorAll('.pendingNameOnly683').forEach(name=>name.classList.remove('pendingNameOnly683'));
  }
 }catch{}
}
...
function pinNav683(){
 ...
 if(nav&&!nav.classList.contains('kmNavFixed683'))nav.classList.add('kmNavFixed683');
}

function cleanQueue683(){
 if(cleaning683)return;
 const box=document.getElementById('queue');
 if(!box)return;
 cleaning683=true;
 try{
  personalRule683(box);
  simplifyPending683(box);
  pinNav683();
 }finally{cleaning683=false}
}

function schedule683(){
 clearTimeout(cleanTimer683);
 cleanTimer683=setTimeout(()=>{
  cleanQueue683();
  requestAnimationFrame(cleanQueue683);
 },0);
}

try{
 const prevQueue683=renderQueue;
 renderQueue=function(...args){
  const r=prevQueue683.apply(this,args);
  cleanQueue683();
  queueMicrotask(cleanQueue683);
  return r;
 };
 window.renderQueue=renderQueue;
}catch(e){console.warn('KokMatch v6.85 queue cleanup wrapper',e)}

try{
 const prevNav683=renderNav;
 renderNav=function(...args){
  const r=prevNav683.apply(this,args);
  pinNav683();
  return r;
 };
 window.renderNav=renderNav;
}catch(e){console.warn('KokMatch v6.85 nav pin wrapper',e)}

try{
 const prevAll683=renderAll;
 renderAll=function(...args){
  const r=prevAll683.apply(this,args);
  pinNav683();
  if(currentView==='queue')schedule683();
  return r;
 };
 window.renderAll=renderAll;
}catch(e){console.warn('KokMatch v6.85 renderAll wrapper',e)}

try{
 const prevGo683=goView;
 goView=function(id,...args){
  const r=prevGo683.call(this,id,...args);
  pinNav683();
  if(id==='queue')schedule683();
  return r;
 };
 window.goView=goView;
}catch(e){console.warn('KokMatch v6.85 goView wrapper',e)}

function arm683(){
 pinNav683();
 const box=document.getElementById('queue');
 if(box&&!observer683){
  observer683=new MutationObserver(ms=>{
   if(cleaning683)return;
   if(ms.some(m=>m.type==='childList'&&(m.addedNodes.length||m.removedNodes.length)))schedule683();
  });
  observer683.observe(box,{childList:true,subtree:true});
 }
 if(currentView==='queue')schedule683();
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',arm683,{once:true});else arm683();
window.addEventListener('pageshow',()=>{pinNav683();if(currentView==='queue')schedule683()},{passive:true});
window.addEventListener('orientationchange',()=>setTimeout(()=>{pinNav683();if(currentView==='queue')cleanQueue683()},120),{passive:true});
if(window.visualViewport){
 visualViewport.addEventListener('resize',()=>pinNav683(),{passive:true});
 visualViewport.addEventListener('scroll',()=>pinNav683(),{passive:true});
}
window.__kokmatchCleanQueue683=cleanQueue683;
})();
