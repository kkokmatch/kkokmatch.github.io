/* KokMatch v6.83 early resume guard: must run before legacy listeners. */
(()=>{
'use strict';
...
window.__kokmatchStandalone='6.83';
window.__kokmatchVersionLock='6.83';
window.__kokmatchLegacyAutoUpdateDisabled=true;
window.__kokmatchQueueStable672='6.83';
try{sessionStorage.setItem('kokmatch_runtime_version','6.83')}catch{}


/* v6.83: persistent group-scoped roster cache for PWA/mobile cold starts. */
(()=>{
...
async function compactState46(){
 ...
 }catch(e){console.warn('compact roster preserve v6.83',e)}
...
function shuttleSvg51(kind){
 ...
 return `<svg viewBox="0 0 24 24" role="img" aria-hidden="true">${defs}
  ...
  <path d="M8.55 16.12 10.02 16.83 21.15 6.35 19.1 4.25Z" fill="${featherFill}" stroke="${dark}" stroke-width=".38"/>
...
function shuttleSvg52(kind){
 ...
 if(kind==='global'){
  return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
   ...
   <path d="M8.75 15.8C12.1 11.4 15.55 7.4 18.6 4.35L20.55 6.2C18.75 9.6 15.2 13.35 10.05 16.83Z" fill="#20c85a" stroke="#126b36" stroke-width=".34"/>
 ...
 if(kind==='manager'){
  return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
   ...
   <path d="M8.75 15.82C12.15 11.35 15.55 7.45 18.7 4.35L20.65 6.25C18.75 9.65 15.25 13.4 10.05 16.83Z" fill="url(#${uid}_gold)" stroke="#835400" stroke-width=".38"/>
 ...
 return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
  ...
  <path d="M8.75 15.82C12.15 11.35 15.55 7.45 18.7 4.35L20.65 6.25C18.75 9.65 15.25 13.4 10.05 16.83Z" fill="${colors.c}" stroke="${colors.edge}" stroke-width=".4"/>
...
/* v6.83 pending ETA: read-only prediction from the current group's canonical state. */
...
/* migrated into v6.0: app-v92.js — v6.83 compacted no-op role observer */
(()=>{
roleLabel=function(r){return r==='admin'?'개발자':r==='manager'?'모임장':r==='organizer'?'운영진':'일반'};
...
function repairMemberControlsV6(){
 // v6.83: obsolete roster rail rewriting disabled; canonical role-safe rail owns this DOM.
 return;
...
const BUILD634=Object.freeze({version:'6.83',label:'콕매치 v6.83 · 최신 운영본'});
...
/* KokMatch v6.83: role-safe canonical member paging */
(()=>{
'use strict';
...
try{memberControls=controlsHtml637;window.memberControls=memberControls}catch(e){console.warn('KokMatch v6.83 memberControls source',e)}
...
try{
 ...
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.83 renderMembers wrapper',e)}
try{
...
}catch(e){console.warn('KokMatch v6.83 member pager wrapper',e)}
function arm637(){
...
/* KokMatch v6.83: no-flash member roster resume */
(()=>{
'use strict';
...
/* KokMatch v6.83: preflight + atomic conflict recovery for multi-organizer game creation */
(()=>{
'use strict';
...
/* KokMatch v6.83: white member/queue cards + right-side queue grade stripe. */
(()=>{
'use strict';
...
/* KokMatch v6.83: personal queue grade watermark only. No card layout mutation. */
(()=>{
'use strict';
...
/* KokMatch v6.83: live operations dashboard inside 운동통계. */
(()=>{
'use strict';
...
/* v6.83: group-scoped AI auto game assignment + pending creator provenance. */
(()=>{
'use strict';
if(window.__kokmatchAutoGame656)return;
window.__kokmatchAutoGame656='6.83';
...
function autoCfg656(){const c=S?.autoGame&&typeof S.autoGame==='object'?S.autoGame:{};return{enabled:c.enabled===true,mode:'priority_v673',updatedAt:Number(c.updatedAt)||0,updatedBy:c.updatedBy&&typeof c.updatedBy==='object'?c.updatedBy:{}}}
...
renderSettings=function(...args){const r=renderSettingsBefore656.apply(this,args);paintAutoSettings656();const box=document.getElementById('settings');if(box)[...box.querySelectorAll('.meta')].forEach(el=>{if(/콕매치 v6\.5[0-5]/.test(el.textContent||''))el.textContent='콕매치 v6.83 · AI 자동게임편성 · 편성자 기록'});return r};
...
async function runAuto656(force=false){
 ...
 try{
  ...
  return x;
 }catch(e){console.warn('auto game v6.83',e);return null}finally{autoBusy656=false}
}
...
/* v6.83 keep today live dashboard mounted */
setTimeout(()=>{if(me){paintAutoSettings656();decorateQueueAuto656()}armAuto656()},0);
})();


/* v6.83: waiting-screen auto controls, clean member badges, developer challenger frame, queue meta alignment. */
(()=>{
'use strict';
if(window.__kokmatchUiRefine658)return;
window.__kokmatchUiRefine658='6.83';
...
/* v6.83 final ordinary-badge, developer-frame and personal queue metadata guard. */
(()=>{
'use strict';
...
/* v6.83 compact auto control + Challenger crest profile frame. */
(()=>{
'use strict';
window.__kokmatchUiRefine659='6.83';
...
window.__kokmatchDevFrame661='6.83';
...
/* v6.83 final roster-card cleanup: no rectangular border/accent survives legacy render styles. */
(()=>{
 'use strict';
...
/* v6.83: manager gold aura + organizer silver aura */
(()=>{
'use strict';
if(window.__kokmatchRoleAura664)return;
window.__kokmatchRoleAura664='6.83';
const MANAGER_SRC='/assets/dev-prism-frame-v662.webp?v=6.83';
const ORGANIZER_SRC='/assets/organizer-silver-aura-v665.webp?v=6.83';
let queued=false;
function role664(id){
...
/* v6.83: automatic mode owns game composition until an authorized operator explicitly switches to manual. */
(()=>{
'use strict';
if(window.__kokmatchAutoManualGuard673)return;window.__kokmatchAutoManualGuard673='6.83';
...
/* v6.83: canonical always-visible developer badge + restored Challenger frame. */
(()=>{
'use strict';
if(window.__kokmatchDeveloperVisible674)return;window.__kokmatchDeveloperVisible674='6.83';
const DEV_FRAME674='/assets/dev-prism-frame-v662.webp?v=6.83';
...
/* v6.83: verified automatic-game controls + stable stats polish. */
(()=>{
'use strict';
if(window.__kokmatchFix675)return;window.__kokmatchFix675='6.83';
...
window.openAutoGameSettings656=function(...args){
 ...
 syncAuto675().catch(e=>console.warn('auto settings sync v6.83',e));
 return r;
};
window.toggleAutoGame656=async function(){
...
/* v6.83: resilient automatic-game toggle; save state and composition execution are separate. */
(()=>{
'use strict';
if(window.__kokmatchAutoToggle676)return;window.__kokmatchAutoToggle676='6.83';
...
window.toggleAutoGame656=async function(){
 ...
 try{
  try{const before=await req676('get');apply676(before);serverBefore=before?.config?.enabled===true}catch(e){console.warn('auto pre-sync v6.83',e)}
  ...
  req676('get').then(v=>{apply676(v);paint676()}).catch(e=>console.warn('auto verify v6.83',e));
  ...
  if(next){
   try{
    ...
    console.warn('auto initial tick v6.83',e);
...
window.openAutoGameSettings656=function(...args){
 ...
 sync676().catch(e=>console.warn('auto settings sync v6.83',e));
 return r;
};
...
/* v6.83: roster no-flash, restored green queue game badge, persistent auto/manual conflict modal. */
(()=>{
'use strict';
if(window.__kokmatchUiStability677)return;window.__kokmatchUiStability677='6.83';
...
/* v6.83: make legacy roster finalizer idempotent so operational polling cannot recreate avatars. */
(()=>{
'use strict';
if(window.__kokmatchRosterFinalizerStable677)return;
const original=window.__kokmatchFinalizeRoster22;
if(typeof original!=='function')return;
window.__kokmatchRosterFinalizerStable677='6.83';
let last='';
function sig677(){
 try{
...
/* v6.83: final member-view renderAll gate. Legacy v22 calls finalizeRoster22 directly, so bypass that chain for operational-only updates. */
(()=>{
'use strict';
if(window.__kokmatchMemberRenderGate677)return;
window.__kokmatchMemberRenderGate677='6.83';
...
/* v6.83: persistent live-operations shell, permanent queue game badge, inline monthly rank circles, mobile roster spacing. */
(()=>{
'use strict';
if(window.__kokmatchUiStability678)return;window.__kokmatchUiStability678='6.83';
...
/* v6.83: visible-shell resilience after PWA/service-worker updates. */
(()=>{
'use strict';
if(window.__kokmatchBootResilience679)return;window.__kokmatchBootResilience679='6.83';
...
function ensureVisibleShell679(){
 ...
 }catch(e){console.warn('KokMatch v6.83 visible shell recovery',e)}
 return false;
}
...
/* v6.83 final court-count focus/draft guard. */
(()=>{
'use strict';
if(window.__kokmatchCourtInputGuard681)return;window.__kokmatchCourtInputGuard681='6.83';
...
try{
 ...
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.83 member render patch',e)}
...
try{
 ...
 window.renderAll=renderAll;
}catch(e){console.warn('KokMatch v6.83 renderAll patch',e)}
...
}catch(e){console.warn('KokMatch v6.83 goView patch',e)}
...
/* v6.83: queue-screen cleanup, fixed bottom navigation guard, and concise pending cards. */
(()=>{
'use strict';
if(window.__kokmatchQueueUx683)return;
window.__kokmatchQueueUx683='6.83';
...
function simplifyPending683(box){
 try{
  ...
  for(const card of cards){
   card.classList.add('pendingSimple683');
   card.querySelectorAll(':scope > .pairSummary').forEach(el=>el.remove());
   card.querySelectorAll('.pendingGrid .pendingSlot:not(.emptySlot)').forEach(slot=>{
    slot.classList.add('pendingPersonSimple683');
    slot.querySelectorAll('.slotBadges53,.slotBadges54,.slotBadges58,.genderMark53,.genderPerson54,.nameGender58,.roleBadge,.inviteReserve54,.inviteSub45,.relationInfo66,.relationEmpty66').forEach(el=>el.remove());
    slot.querySelectorAll('.slotName .tag,.slotName .gradeBadge50,.slotName .gamecnt').forEach(el=>el.remove());
    slot.querySelectorAll(':scope > .meta,:scope > .compactMeta53').forEach(el=>el.remove());
    const name=slot.querySelector('.slotName');
    if(name){
     name.classList.add('pendingNameOnly683');
     const compact=name.querySelector('.compactName53');
     if(compact)compact.textContent=String(compact.textContent||'').trim();
    }
   });
  }
 }catch{}
...
}catch(e){console.warn('KokMatch v6.83 queue cleanup wrapper',e)}
...
}catch(e){console.warn('KokMatch v6.83 nav pin wrapper',e)}
...
try{
 ...
 window.renderAll=renderAll;
}catch(e){console.warn('KokMatch v6.83 renderAll wrapper',e)}
...
try{
 ...
 window.goView=goView;
}catch(e){console.warn('KokMatch v6.83 goView wrapper',e)}
