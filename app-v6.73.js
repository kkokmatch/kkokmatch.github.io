/* KokMatch v6.73 early resume guard: must run before legacy listeners. */
(()=>{
'use strict';
...
window.__kokmatchStandalone='6.73';
window.__kokmatchVersionLock='6.73';
window.__kokmatchLegacyAutoUpdateDisabled=true;
window.__kokmatchQueueStable672='6.73';
try{sessionStorage.setItem('kokmatch_runtime_version','6.73')}catch{}


/* v6.73: persistent group-scoped roster cache for PWA/mobile cold starts. */
(()=>{
...
async function compactState46(){
 ...
 }catch(e){console.warn('compact roster preserve v6.73',e)}
...
function shuttleSvg51(kind){
 ...
 const cork=`<g>
  ...
  <path d="M6.35 16.73 9.15 18.18" stroke="#72685C" stroke-width="2.2" stroke-linecap="round"/>
  <path d="M6.73 16.2 9.28 17.9" stroke="#E8DDC9" stroke-width="1.15" stroke-linecap="round"/>
 </g>`;
 const frame=`<path d="M8.05 15.55 15.4 3.35M8.65 16.05 18.5 4.25M9.2 16.73 21.35 7.15" fill="none" stroke="#5F5B56" stroke-opacity=".55" stroke-width=".56" stroke-linecap="round"/>
  <path d="M8.05 15.55 9.25 16.73 10.1 17.05" fill="none" stroke="#6B6258" stroke-width=".65" stroke-linecap="round"/>`;
 if(kind==='global'){
  return `<svg viewBox="0 0 24 24" role="img" aria-hidden="true">${defs}
   ...
   <path d="M8.05 15.75 9.62 16.73 18.45 4.05 16.15 3.15Z" fill="#FFB000" stroke="#8A6100" stroke-width=".35"/>
 ...
 return `<svg viewBox="0 0 24 24" role="img" aria-hidden="true">${defs}
  ...
  <path d="M8.05 15.82 9.66 16.73 19.1 4.25 16.1 3.2Z" fill="${secondary}" stroke="${dark}" stroke-width=".38"/>
  <path d="M8.55 16.12 10.02 16.82 21.15 6.35 19.1 4.25Z" fill="${featherFill}" stroke="${dark}" stroke-width=".38"/>
  <path d="M8.95 16.73 10.25 17.08 22.15 9.2 21.15 6.35Z" fill="${secondary}" stroke="${dark}" stroke-width=".38"/>
...
function shuttleSvg52(kind){
 ...
 const ribs=`<path d="M8.6 15.75 15.3 3.55M9.2 16.15 18.3 4.35M9.72 16.73 21.05 6.3" fill="none" stroke="#5d5954" stroke-opacity=".58" stroke-width=".58" stroke-linecap="round"/>
 ...
 if(kind==='global'){
  return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
   ...
   <path d="M8.25 15.42C10.7 10.55 13.25 6.3 16 3.35L18.6 4.35C17.25 8.25 14.25 12.8 9.72 16.73Z" fill="#ffb000" stroke="#8a6100" stroke-width=".34"/>
 ...
 if(kind==='manager'){
  return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
   ...
   <path d="M8.25 15.45C10.7 10.55 13.3 6.25 16.05 3.35L18.7 4.35C17.25 8.25 14.3 12.85 9.72 16.73Z" fill="url(#${uid}_gold)" stroke="#835400" stroke-width=".38"/>
 ...
 return `<svg viewBox="0 0 24 24" aria-hidden="true">${defs}
  ...
  <path d="M8.25 15.45C10.7 10.55 13.3 6.25 16.05 3.35L18.7 4.35C17.25 8.25 14.3 12.85 9.72 16.73Z" fill="${colors.b}" stroke="${colors.edge}" stroke-width=".4"/>
...
/* migrated into v6.0: app-v92.js — v6.73 compacted no-op role observer */
(()=>{
roleLabel=function(r){return r==='admin'?'개발자':r==='manager'?'모임장':r==='organizer'?'운영진':'일반'};
...
function repairMemberControlsV6(){
 // v6.73: obsolete roster rail rewriting disabled; canonical role-safe rail owns this DOM.
 return;
...
const BUILD634=Object.freeze({version:'6.73',label:'콕매치 v6.73 · 최신 운영본'});
...
/* KokMatch v6.73: role-safe canonical member paging */
(()=>{
'use strict';
...
try{memberControls=controlsHtml637;window.memberControls=memberControls}catch(e){console.warn('KokMatch v6.73 memberControls source',e)}
...
try{
 ...
 window.renderMembers=renderMembers;
}catch(e){console.warn('KokMatch v6.73 renderMembers wrapper',e)}
try{
...
}catch(e){console.warn('KokMatch v6.73 member pager wrapper',e)}
function arm637(){
...
/* KokMatch v6.73: no-flash member roster resume */
(()=>{
'use strict';
...
/* KokMatch v6.73: preflight + atomic conflict recovery for multi-organizer game creation */
(()=>{
'use strict';
...
/* KokMatch v6.73: white member/queue cards + right-side queue grade stripe. */
(()=>{
'use strict';
...
/* KokMatch v6.73: personal queue grade watermark only. No card layout mutation. */
(()=>{
'use strict';
...
/* KokMatch v6.73: live operations dashboard inside 운동통계. */
(()=>{
'use strict';
...
/* v6.73: group-scoped AI auto game assignment + pending creator provenance. */
(()=>{
'use strict';
if(window.__kokmatchAutoGame656)return;
window.__kokmatchAutoGame656='6.73';
...
renderSettings=function(...args){const r=renderSettingsBefore656.apply(this,args);paintAutoSettings656();const box=document.getElementById('settings');if(box)[...box.querySelectorAll('.meta')].forEach(el=>{if(/콕매치 v6\.5[0-5]/.test(el.textContent||''))el.textContent='콕매치 v6.73 · AI 자동게임편성 · 편성자 기록'});return r};
...
async function runAuto656(force=false){
 ...
 try{
  ...
  return x;
 }catch(e){console.warn('auto game v6.73',e);return null}finally{autoBusy656=false}
}
...
/* v6.73: waiting-screen auto controls, clean member badges, developer challenger frame, queue meta alignment. */
(()=>{
'use strict';
if(window.__kokmatchUiRefine658)return;
window.__kokmatchUiRefine658='6.73';
...
/* v6.73 final ordinary-badge, developer-frame and personal queue metadata guard. */
(()=>{
'use strict';
...
/* v6.73 compact auto control + Challenger crest profile frame. */
(()=>{
'use strict';
window.__kokmatchUiRefine659='6.73';
...
window.__kokmatchDevFrame661='6.73';
...
/* v6.73 final roster-card cleanup: no rectangular border/accent survives legacy render styles. */
(()=>{
 'use strict';
...
/* v6.73: manager gold aura + organizer silver aura */
(()=>{
'use strict';
if(window.__kokmatchRoleAura664)return;
window.__kokmatchRoleAura664='6.73';
const MANAGER_SRC='/assets/dev-prism-frame-v662.webp?v=6.73';
const ORGANIZER_SRC='/assets/organizer-silver-aura-v665.webp?v=6.73';
let queued=false;
function role664(id){
