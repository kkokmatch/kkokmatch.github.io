...
window.__kokmatchStandalone='6.23';
window.__kokmatchVersionLock='6.23';
window.__kokmatchLegacyAutoUpdateDisabled=true;
try{sessionStorage.setItem('kokmatch_runtime_version','6.23')}catch{}
...
renderSettings=function(){
 ...
 [...box.querySelectorAll('.meta')].forEach(el=>{if((el.textContent||'').includes('콕매치 v90'))el.textContent='콕매치 v91 · 전 화면 가독성 확대 · 파트너 표시 개선'});
};
...
renderSettings=function(){settingsBefore18();const b=$('settings');if(!b)return;[...b.querySelectorAll('.meta')].forEach(el=>{if(/콕매치 v(?:\d+|1\.[0-9]+)/.test(el.textContent||''))el.textContent='콕매치 v1.8 · 회원검색 입력 안정화'});};
